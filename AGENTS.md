...
- **Tool**: `packages/agent-core/src/tools/builtin/collaboration/contact-parent.ts` — `ContactParent` (≤4 accepted requests per child turn; same-key requests deduped within a turn). Mounted for every subagent (`type === 'sub'`) and gated on `agent.ownerHost`. Handoff payloads carry `artifacts` / `evidence` / `missing` plus `expecting` (what a good reply looks like), rendered into the parent notification body.
...
- **Profile-gated tools**: `Agent.useProfile` rebuilds the builtin registry after it writes `profileName` and the active tool names. `canSpawn` reads the `spawns` whitelist off `config.profileName` and `setActiveTools` only records names, so without that second pass a freshly spawned `plan`/`reviewer` child — whose spawn-time rebuild runs *before* its profile is applied — never received its `Agent` tool (`loopToolsFor` drops unregistered names). The rebuild keeps the same `config.hasProvider` guard as `ConfigState.update`, so provider-less agents (some tests, migrated sessions) cannot throw while building tools.
...
- **TUI surface**: the same call also cuts into the main transcript as an *interjection* row — `apps/scream-code/src/tui/utils/child-request-notice.ts` formats it, and `INTERJECTION_BULLET` (`▸ `, warning tint, in `apps/scream-code/src/tui/constant/symbols.ts`) keeps it distinct from the `■ ` used for user and assistant speech. Plain notices (command feedback, approval results) stay unmarked. Session replay re-derives the row from the injected notification — name-less, because spawn events are not persisted — and every other `system_trigger` injection is deliberately not rendered.
...
- **Parent→child delivery** (`SendSubagentMessage`): a `steer` sent while the child's turn is live is injected into that turn through `Turn.steer`, so it joins at the child's next step boundary (`flushSteerBuffer`) instead of waiting for the turn to end. The origin is `system_trigger`, not `user`, so `Turn.hasPendingSteer` never aborts the tool call in flight. Mid-run injection is capped by the same in-flight budget as the mailbox (`DEFAULT_IN_FLIGHT_LIMIT`, read through `Turn.steerQueueLength`); a full buffer falls back to the mailbox. Retried sends are deduplicated per child turn — but a key is only registered after the message actually lands, so a retry after a rejection (mailbox full, deadline elapsed) is attempted honestly. Structured-output children are the exception — their steers also wait for the bounded delivery turn, whose JSON guard (`STRUCTURED_MESSAGE_DELIVERY_PROMPT`) keeps the machine-readable contract intact. Everything else waits in `SubagentMessageBus` and is injected at the child's next turn start; `runChild` keeps a delivery budget (`MAX_PARENT_MESSAGE_DELIVERY_TURNS`) separate from the summary-expansion budget, because sharing one counter let a short summary spend the only window a queued message had. A mailbox holds up to `DEFAULT_IN_FLIGHT_LIMIT` (4) undelivered messages, each deliverable for `SUBAGENT_MESSAGE_DEADLINE_MS` (5 min); `activeCount` ignores expired ones so a dead message cannot burn a delivery turn, and a finished run never purges its mailbox — expiry or a later resume settles it. The acknowledgement names the path taken (`mid-run` / `queued`). A steer that lands while the turn's Stop hook is awaiting is re-flushed before the turn ends (`shouldContinueAfterStop`), so it cannot be destroyed by teardown after the parent already saw "delivered".
...
- **Tool**: `packages/agent-core/src/tools/builtin/collaboration/wolfpack.ts` — `WolfPackTool`, runtime-gated by `wolfpackMode.isActive`. Its description carries the same generated roster as `Agent` (`buildSubagentDescriptions`, sourced from `agent.yaml` `subagents.*.description` plus each profile's `whenToUse`), so the per-type USE WHEN / NOT FOR text drives batch selection too.
...
The Web frontend hydrates Goal/Todo only from session snapshots and full `goal.updated` / `todo.updated` journal events. Its session detail drawer (opened from the conversation header) keeps the existing quick actions above a complete Goal manager and a read-only, phase-grouped core Todo view; session/connection generations plus epoch/seq guards prevent stale snapshots and mutation responses from crossing session switches or reconnects.
