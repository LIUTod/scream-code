---
name: release
description: Execute the release workflow for Scream Code CLI packages.
---

Release process for Scream Code CLI packages. Tags are pushed without a `v` prefix and follow one of these patterns (matched by `.github/workflows/release-*.yml`):

| Tag pattern | Releases |
|---|---|
| `1.42.0` (numeric) | `scream-cli` (root) + `scream-code` wrapper, released together — versions must stay aligned |
| `ltod-0.53.0` | `packages/ltod` |
| `pykaos-0.9.0` | `packages/kaos` (PyPI name `pykaos`) |
| `scream-sdk-0.3.0` | `sdks/scream-sdk` |
...
2. **Detect changed packages.** Check each release unit under `packages/`, `sdks/`, and the repo root for changes since its last release tag. Use path-scoped diffs for subpackages so unrelated repo changes do not trigger a package release, e.g. `git diff ltod-0.53.0..HEAD -- packages/ltod`, `git diff pykaos-0.9.0..HEAD -- packages/kaos`, and `git diff scream-sdk-0.2.1..HEAD -- sdks/scream-sdk`. If nothing changed anywhere, stop and report that there is nothing to release. Note: `packages/scream-code` is a thin wrapper and must stay version-synced with `scream-cli`, so treat it as changed whenever the root package changes.
...
5. **Update version metadata and changelogs.** For each changed package:
   ...
   - If bumping `packages/ltod` or `packages/kaos`, also update the root `pyproject.toml` pinned dependency (`ltod[contrib]==<version>` or `pykaos==<version>`) so root validation keeps passing.

6. **Sync the `scream-code` wrapper when the root version changes.** Bump `packages/scream-code/pyproject.toml` `version` and its `scream-cli==<version>` dependency to match the new root version.
...
9. **Confirm with the user before opening the PR.** Summarize the staged changes and ask the user to explicitly confirm:
   - **Version numbers** — every updated `pyproject.toml` (changed package + `packages/scream-code` if the root moved) reflects the version agreed in step 3.
   - **Dependency pins** — the root `pyproject.toml` pins (`ltod[contrib]==<version>`, `pykaos==<version>`) and `packages/scream-code`'s `scream-cli==<version>` match the bumped versions.
...
10. **Open the PR.** Commit all changes, push, and open a PR with `gh`. The PR description must follow this structure (see https://github.com/MoonshotAI/scream-cli/pull/2225 for reference):
    ...
    - (For root releases) uv run python scripts/check_version_tag.py --pyproject packages/scream-code/pyproject.toml --expected-version <version>
    - uv run python scripts/check_scream_dependency_versions.py --root-pyproject pyproject.toml --ltod-pyproject packages/ltod/pyproject.toml --pykaos-pyproject packages/kaos/pyproject.toml
    - make check
...
11. **Hand off the tag step.** After merge, switch to `main`, pull latest, and tell the user the exact `git tag` command for the final release tag (pick the right tag pattern from the table above — e.g. `git tag 1.43.0` for a root release, `git tag ltod-0.54.0` for a ltod release). The user will run the tag and push tags themselves.
