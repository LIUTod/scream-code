...
import { JianError, JianFileExistsError, JianValueError } from './errors';
...
/**
 * Advanced ssh2 connect options that may be passed through `SSHJianOptions.extraOptions`.
 *
 * Excludes fields that SSHJian manages itself (`host`, `port`, `username`,
 * `password`, `privateKey`, `authHandler`, `hostVerifier`) — those are derived
 * from the top-level `SSHJianOptions` fields and cannot be overridden here.
 */
export type SSHJianExtraOptions = Omit<
  ConnectConfig,
  'host' | 'port' | 'username' | 'password' | 'privateKey' | 'authHandler' | 'hostVerifier'
>;

export interface SSHJianOptions {
  host: string;
  port?: number;
  username: string;
  password?: string;
   ...
   */
  extraOptions?: SSHJianExtraOptions;
}

// ── SSH error types ───────────────────────────────────────────────────

export class JianSSHError extends JianError {
  readonly code: number | undefined;

  constructor(message: string, code?: number) {
    super(message);
    this.name = 'JianSSHError';
    this.code = code;
  }
}

export class JianFileNotFoundError extends JianSSHError {
  constructor(message: string, code?: number) {
    super(message, code);
    this.name = 'JianFileNotFoundError';
  }
}

export class JianPermissionError extends JianSSHError {
  constructor(message: string, code?: number) {
    super(message, code);
    this.name = 'JianPermissionError';
  }
}

export class JianConnectionError extends JianSSHError {
  constructor(message: string, code?: number) {
    super(message, code);
    this.name = 'JianConnectionError';
  }
}
...
function mapSftpError(operation: string, error: unknown): JianSSHError {
  ...
  if (code === statusCode.NO_SUCH_FILE) {
    return new JianFileNotFoundError(message, code);
  }
  if (code === statusCode.PERMISSION_DENIED) {
    return new JianPermissionError(message, code);
  }
  if (code === statusCode.NO_CONNECTION || code === statusCode.CONNECTION_LOST) {
    return new JianConnectionError(message, code);
  }
  return new JianSSHError(message, code);
}
...
export class SSHProcess implements JianProcess {
...
// callers see a JianSSHError subclass (JianFileNotFoundError / JianPermissionError /
// JianConnectionError / generic JianSSHError) instead of the raw ssh2 error.
...
/**
 * A JIAN implementation that interacts with a remote machine via SSH and SFTP.
 */
export class SSHJian implements Jian {
  ...
  get osEnv(): Environment {
    throw new JianError(
      'SSHJian.osEnv is not yet wired — remote environment probing is not implemented.',
    );
  }
  ...
  withCwd(cwd: string): SSHJian {
    return new SSHJian(this._client, this._sftp, this._home, cwd);
  }
  ...
  /**
   * Factory method to create an SSHJian instance.
   * Establishes the SSH connection and SFTP session.
   */
  static async create(options: SSHJianOptions): Promise<SSHJian> {
    ...
    try {
      ...
      if (options.cwd === undefined) {
        ...
        if (!attrs.isDirectory()) {
          throw new JianValueError(`${cwd} is not a directory`);
        }
      }

      return new SSHJian(client, sftp, home, cwd);
    } catch (error) {
      client.end();
      throw error;
    }
  ...
  async chdir(path: string): Promise<void> {
    ...
    if (!attrs.isDirectory()) {
      throw new JianValueError(`${resolved} is not a directory`);
    }
    this._cwd = resolved;
  }
  ...
  async *glob(
    ...
    if (!caseSensitive) {
      throw new JianValueError('Case insensitive glob is not supported in current environment');
    }
  ...
  private async _mkdirRecursive(path: string, existOk: boolean): Promise<void> {
    ...
    for (const [i, part] of parts.entries()) {
      ...
      if (exists) {
        ...
        if (isFinal && !existOk) {
          throw new JianFileExistsError(`${current} already exists`);
        }
        ...
        if (!st.isDirectory()) {
          throw new JianFileExistsError(`${current} already exists but is not a directory`);
        }
        continue;
      }
      try {
  ...
  exec(...args: string[]): Promise<JianProcess> {
    if (args.length === 0) {
      throw new JianValueError(
        'SSHJian.exec(): at least one argument (the command to run) is required.',
      );
    }
  ...
  execWithEnv(args: string[], env?: Record<string, string>): Promise<JianProcess> {
    if (args.length === 0) {
      throw new JianValueError(
        'SSHJian.execWithEnv(): at least one argument (the command to run) is required.',
      );
    }
    ...
    if (env !== undefined) {
      ...
      for (const [key, value] of Object.entries(env)) {
        ...
        if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(key)) {
          throw new JianValueError(
            `SSHJian.execWithEnv(): invalid env variable name ${JSON.stringify(key)}`,
          );
        }
  ...
  private async _execInternal(args: string[], env?: Record<string, string>): Promise<JianProcess> {
    const command = SSHJian._buildExecCommand(args, this._cwd, env);
  ...
  /**
   * Close the SSH connection. After this, the SSHJian instance is unusable.
   */
  close(): Promise<void> {
    this._sftp.end();
