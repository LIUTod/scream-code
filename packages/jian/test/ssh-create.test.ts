...
import type { SSHJian as SSHJianType } from '#/ssh';
...
describe('SSHJian.create()', () => {
  it('initializes cwd equal to gethome() when no cwd option is passed', async () => {
    // Pins the Python test_ssh_jian.py::test_pathclass_home_and_cwd invariant:
    ...
    const { SSHJian } = await loadSSHModule();

    const ssh = await SSHJian.create({
      host: 'example.com',
      username: 'tester',
    });
  ...
  it('rejects a cwd that resolves to a regular file', async () => {
    ...
    const { SSHJian, state } = await loadSSHModule({ sftp });

    const error = await SSHJian.create({
      host: 'example.com',
      username: 'tester',
      cwd: 'file.txt',
    ...
    expect((error as Error).name).toBe('JianValueError');
  ...
  it('tries multiple private keys via authHandler until one succeeds', async () => {
    const { SSHJian, state } = await loadSSHModule({
      readFileValues: {
