...
import { JianFileNotFoundError, SSHJian } from '#/ssh';

// ── SSH path resolution: mock SFTP harness ────────────────────────────
//
// This test file validates that SSHJian file/dir operations correctly
...
// returns a deterministic response. After each SSHJian operation we
// inspect the recorded path to ensure the resolution worked.
//
// The private SSHJian constructor is bypassed via `Reflect.construct` —
...
function createMockSftp(state: MockSFTP): SFTPWrapper {
  ...
  const sftp = {
    realpath(path: string, cb: (err: unknown, absPath: string) => void): void {
      ...
      // Otherwise it would be relative — but since SSHJian always
...
/**
 * Construct an SSHJian with the private constructor bypassed. TS
 ...
 */
function createMockedJian(sftp: SFTPWrapper, home: string, cwd: string): SSHJian {
  const client = createMockClient();
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const CtorAny = SSHJian as any;
  // eslint-disable-next-line @typescript-eslint/no-unsafe-call
  return new CtorAny(client, sftp, home, cwd) as SSHJian;
}

// ── Tests ─────────────────────────────────────────────────────────────

describe('e2e: SSHJian relative path resolution after chdir (mocked SFTP)', () => {
  let state: MockSFTP;
  ...
  beforeEach(() => {
    ...
    jian = createMockedJian(sftp, '/home/user', '/home/user');
  });
  ...
  describe('readText / readBytes / readLines', () => {
    it('readText resolves relative path against current cwd', async () => {
      ...
      await jian.chdir('/remote/tmp');

      state.files.set('/remote/tmp/file.txt', Buffer.from('hello'));

      const text = await jian.readText('file.txt');
    ...
    it('readText accepts absolute path unchanged', async () => {
      ...
      const text = await jian.readText('/etc/hosts');
    ...
    it('readBytes resolves relative path against current cwd', async () => {
      ...
      await jian.chdir('/opt/data');
      state.files.set('/opt/data/blob.bin', Buffer.from([1, 2, 3, 4]));

      const bytes = await jian.readBytes('blob.bin');
    ...
    it('readLines resolves relative path against current cwd', async () => {
      ...
      await jian.chdir('/var/log');
      state.files.set('/var/log/app.log', Buffer.from('line1\nline2\nline3'));

      const lines: string[] = [];
      for await (const line of jian.readLines('app.log')) {
        lines.push(line);
      }
    ...
    it('readLines strips CRLF terminators', async () => {
      ...
      const lines: string[] = [];
      for await (const line of jian.readLines('app-crlf.log')) {
        lines.push(line);
      }
  ...
  describe('writeText / writeBytes / appendFile', () => {
    it('writeText resolves relative path against current cwd', async () => {
      ...
      await jian.chdir('/workspace');

      const n = await jian.writeText('out.txt', 'hello world');
    ...
    it('writeBytes resolves relative path against current cwd', async () => {
      ...
      const n = await jian.writeBytes('blob.bin', data);
      expect(n).toBe(4);
    ...
    it('writeText with mode=a uses appendFile and resolves path', async () => {
      state.dirs.add('/logs');
      await jian.chdir('/logs');

      await jian.writeText('a.log', 'line1\n', { mode: 'a' });
      await jian.writeText('a.log', 'line2\n', { mode: 'a' });
  ...
  describe('stat / mkdir', () => {
    it('stat resolves relative path against current cwd', async () => {
      state.dirs.add('/srv');
      await jian.chdir('/srv');
      state.files.set('/srv/config.json', Buffer.from('{}'));

      const result = await jian.stat('config.json');
    ...
    it('stat preserves NO_SUCH_FILE as JianFileNotFoundError', async () => {
      state.dirs.add('/srv');
      await jian.chdir('/srv');

      await expect(jian.stat('missing.json')).rejects.toBeInstanceOf(JianFileNotFoundError);
      await expect(jian.stat('missing.json')).rejects.toMatchObject({ code: 2 });
    });
    ...
    it('stat with followSymlinks=false uses lstat and resolves path', async () => {
      ...
      await jian.stat('link', { followSymlinks: false });
    ...
    it('mkdir with parents=true resolves and creates intermediate dirs', async () => {
      ...
      await jian.mkdir('a/b/c', { parents: true });
    ...
    it('mkdir existOk=true is idempotent against the resolved path', async () => {
      ...
      await expect(jian.mkdir('existing', { existOk: true })).resolves.toBeUndefined();

      // Without existOk, it should throw.
      await expect(jian.mkdir('existing', { existOk: false })).rejects.toThrow();
    });
    ...
    it('mkdir with parents=true rejects a raced file collision when existOk=true', async () => {
      ...
      await expect(
        jian.mkdir('collision', { parents: true, existOk: true }),
      ).rejects.toBeInstanceOf(JianFileExistsError);
    });
  });
  ...
  describe('iterdir', () => {
    ...
    it('iterdir with named relative subdir resolves under cwd', async () => {
      ...
      const entries: string[] = [];
      for await (const entry of jian.iterdir('sub')) {
        entries.push(entry);
      }
  ...
  describe('chdir follows realpath and affects subsequent relative ops', () => {
    it('consecutive chdir + readText rebase the working directory each time', async () => {
      ...
      await jian.chdir('/a');
      expect(await jian.readText('one.txt')).toBe('A');

      await jian.chdir('/b');
      expect(await jian.readText('two.txt')).toBe('B');

      // After chdir(/b), a relative path that only exists under /a
      // must NOT resolve — confirming we've truly rebased.
      await expect(jian.readText('one.txt')).rejects.toThrow();
    });
    ...
    it('chdir with a relative argument resolves against the prior cwd', async () => {
      ...
      await jian.chdir('/home/user');
      await jian.chdir('project');

      expect(jian.getcwd()).toBe('/home/user/project');

      state.files.set('/home/user/project/README.md', Buffer.from('readme'));
      expect(await jian.readText('README.md')).toBe('readme');
    });
  });
});
