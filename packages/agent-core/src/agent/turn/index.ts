...
export class TurnFlow {
  ...
  private launch(input: readonly ContentPart[], origin: PromptOrigin): number | null {
    ...
    // prompt) lives in `runOneTurn`, so a goal-driven run emits a clean
...
function classifyApiError(error: unknown, summary: ScreamErrorPayload): ApiErrorClassification {
...
function summaryStatusCode(summary: ScreamErrorPayload): number | undefined {
  const statusCode = summary.details?.['statusCode'];
  return typeof statusCode === 'number' ? statusCode : undefined;
}

function isApiConnectionError(error: unknown, summary: ScreamErrorPayload): boolean {
  return error instanceof APIConnectionError || summary.name === 'APIConnectionError';
}

function isApiTimeoutError(error: unknown, summary: ScreamErrorPayload): boolean {
  return (
...
function isApiEmptyResponseError(error: unknown, summary: ScreamErrorPayload): boolean {
