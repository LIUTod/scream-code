...
export class TurnFlow {
  ...
  private async driveGoal(
    ...
    const effectiveMaxGoalTurns = configuredMaxGoalTurns ?? TURN_DEFAULTS.maxGoalTurns;
        ...
        const result = await runTurn({
          ...
          hooks: {
            ...
            shouldContinueAfterStop: async ({ signal, stopReason }) => {
              ...
              if (this.convergenceInjections < TURN_DEFAULTS.maxConvergenceInjections) {
...
[system: verification skipped — identical successful run within the last ${Math.round(
  ...
  private lastAssistantMessageIsTrivial(): boolean {
    ...
    for (let i = history.length - 1; i >= 0; i--) {
      ...
      return (
        trimmed.length < TURN_DEFAULTS.minFinalResponseLength ||
