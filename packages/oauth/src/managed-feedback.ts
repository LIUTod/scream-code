/**
 * Submit user feedback to the managed Scream Code platform.
 *
 * POSTs a JSON body to `{screamCodeBaseUrl}/feedback` with a Bearer access
 * token. The client tags `version` with a `scream-code-` prefix so the
...
export function screamCodeFeedbackUrl(): string {
  return `${screamCodeBaseUrl().replace(/\/+$/, '')}/feedback`;
}
