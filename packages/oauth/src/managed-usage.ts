 ...
 *
 * Only `managed:scream-code` is supported today. The platform exposes a
...
const SCREAM_CODE_PLATFORM_ID = 'scream-code';

export function isManagedScreamCode(providerKey?: string | null): boolean {
  if (!providerKey) return false;
  if (!providerKey.startsWith(MANAGED_PREFIX)) return false;
  return providerKey.slice(MANAGED_PREFIX.length) === SCREAM_CODE_PLATFORM_ID;
}

export function screamCodeBaseUrl(): string {
  return process.env['SCREAM_CODE_BASE_URL'] ?? 'https://api.scream.com/coding/v1';
}

export function screamCodeUsageUrl(): string {
  return `${screamCodeBaseUrl().replace(/\/+$/, '')}/usages`;
}
    ...
    if (!res.ok) {
      ...
      const hint =
        status === 401
          ...
          : status === 404
            ? 'Usage endpoint not available. Try Scream For Coding.'
