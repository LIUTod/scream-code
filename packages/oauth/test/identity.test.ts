...
function tempHome(): string {
  const dir = mkdtempSync(join(tmpdir(), 'scream-oauth-identity-'));
  tmpRoots.push(dir);
  return dir;
}
...
describe('Scream identity factories', () => {
  it('creates and reuses a device id in the explicit homeDir', () => {
    const homeDir = tempHome();
    const first = createScreamDeviceId(homeDir);
    const second = createScreamDeviceId(homeDir);
  ...
  it('creates different device ids for different homeDir values', () => {
    const first = createScreamDeviceId(tempHome());
    const second = createScreamDeviceId(tempHome());

    expect(second).not.toBe(first);
  });

  it('creates complete X-Msh device headers from host version', () => {
    const headers = createScreamDeviceHeaders({
      homeDir: tempHome(),
      version: '1.2.3-test',
    });
  ...
  it('creates scream-code-cli User-Agent and appends suffix only to UA', () => {
    expect(
      createScreamUserAgent({
        userAgentProduct: 'scream-code-cli',
        version: '1.2.3',
      }),
    ).toBe('scream-code-cli/1.2.3');
    expect(
      createScreamUserAgent({
        userAgentProduct: 'scream-code-cli',
        version: '1.2.3',
    ...
    ).toBe('scream-code-cli/1.2.3 (wire 4.5.6)');
  });
...
describe('ascii header value sanitization', () => {
  it('strips a trailing newline from a header value', () => {
    const ua = createScreamUserAgent({ userAgentProduct: 'scream-code-cli', version: '6.8.0-101\n' });
    expect(ua).toBe('scream-code-cli/6.8.0-101');
  });

  it('drops non-ASCII codepoints while keeping the ASCII remainder', () => {
    const ua = createScreamUserAgent({ userAgentProduct: 'scream-code-cli', version: 'héllo' });
    expect(ua).toBe('scream-code-cli/hllo');
  });
  ...
  it('uses the unknown fallback when every hostname codepoint is non-ASCII', async () => {
    ...
    try {
      const { createScreamDeviceHeaders: createHeaders } = await import('../src/identity');
  ...
  it('falls back to Darwin kernel version when sw_vers is unavailable', async () => {
    ...
    try {
      const { createScreamDeviceHeaders } = await import('../src/identity');
      const headers = createScreamDeviceHeaders({ homeDir: tempHome(), version: '1.0.0' });
