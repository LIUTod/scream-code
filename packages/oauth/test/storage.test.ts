...
function makeTmpDir(): string {
  const dir = join(
    tmpdir(),
    `scream-storage-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
...
describe('FileTokenStorage', () => {
  ...
  it('returns undefined when no token exists', async () => {
    expect(await storage.load('scream-code')).toBeUndefined();
  });

  it('round-trips a token via save/load', async () => {
    const token = sampleToken();
    await storage.save('scream-code', token);
    const loaded = await storage.load('scream-code');
  ...
  it('persists tokens in snake_case JSON (Python-compatible)', async () => {
    const token = sampleToken();
    await storage.save('scream-code', token);
    const raw = readFileSync(join(dir, 'scream-code.json'), 'utf-8');
  ...
  it('writes the credentials file with mode 0600', async () => {
    await storage.save('scream-code', sampleToken());
    const stat = statSync(join(dir, 'scream-code.json'));
  ...
  it('remove() deletes the file; load() then returns undefined', async () => {
    await storage.save('scream-code', sampleToken());
    await storage.remove('scream-code');
    expect(await storage.load('scream-code')).toBeUndefined();
  });
  ...
  it('save() overwrites an existing token atomically', async () => {
    await storage.save('scream-code', sampleToken({ accessToken: 'first' }));
    await storage.save('scream-code', sampleToken({ accessToken: 'second' }));
    const loaded = await storage.load('scream-code');
    expect(loaded?.accessToken).toBe('second');
  });

  it('load() returns undefined on corrupt JSON (does not throw)', async () => {
    const file = join(dir, 'scream-code.json');
  ...
  it('load() tolerates missing numeric fields by defaulting to 0', async () => {
    ...
    chmodSync(file, 0o600);
    const token = await storage.load('scream-code');
  ...
  it('list() returns all stored token names', async () => {
    ...
    expect(names.toSorted()).toEqual(['other-provider', 'scream-code']);
  });

  it('list() ignores non-JSON files in the credentials dir', async () => {
    await storage.save('scream-code', sampleToken());
    writeFileSync(join(dir, 'scream-code.lock'), 'lock', 'utf-8');
    writeFileSync(join(dir, 'readme.txt'), 'readme', 'utf-8');
    const names = await storage.list();
    expect(names).toEqual(['scream-code']);
  });
  ...
  it('creates the credentials dir with mode 0700 if missing', async () => {
    ...
    await s.save('scream-code', sampleToken());
  ...
  it('save() leaves no *.tmp.* sibling once the rename completes', async () => {
    ...
    const tmps = entries.filter((name) => name.startsWith('scream-code.json.tmp.'));
    expect(tmps).toEqual([]);
    expect(entries).toContain('scream-code.json');
  });
