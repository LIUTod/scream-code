 ...
 * `proper-lockfile` are bypassed via the `SCREAM_DISABLE_OAUTH_LOCK=1`
...
const WORKER_SCRIPT = `
  ...
  import { join } from 'node:path';
  const { OAuthManager } = await import(process.env.SCREAM_OAUTH_ENTRY);

  const shareDir = process.env.SCREAM_CODE_HOME;
  ...
  async function waitForFirstLoadBarrier() {
    if (process.env.SCREAM_SYNC_FIRST_LOAD !== '1') return;
    await appendFile(readyPath, '.');
    const expected = Number(process.env.SCREAM_WORKER_COUNT || '1');
  ...
  if (process.env.DEBUG_OAUTH_WORKER === '1') {
    process.stderr.write('[worker ' + process.env.SCREAM_WORKER_ID + '] done\\n');
  }
`;
...
describe.skipIf(skipOnWindows)('OAuthManager cross-process refresh lock', () => {
  it('2 workers concurrently force-refresh → exactly one refreshImpl fires', async () => {
    ...
    const workers = await spawnInlineWorkers({
      ...
      timeoutMs: 30_000,
      env: {
        SCREAM_OAUTH_ENTRY: OAUTH_ENTRY_URL,
        SCREAM_SYNC_FIRST_LOAD: '1',
        SCREAM_WORKER_COUNT: '2',
      },
    });
...
describe.skipIf(!skipOnWindows)('OAuthManager cross-process refresh lock (Windows skip)', () => {
  it('skipped on Windows — covered by SCREAM_DISABLE_OAUTH_LOCK=1 env escape hatch', () => {
