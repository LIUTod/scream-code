...
describe('screamCodeFeedbackUrl', () => {
  it('appends /feedback to the default base URL', () => {
    expect(screamCodeFeedbackUrl()).toBe('https://api.scream.com/coding/v1/feedback');
  });

  it('honours SCREAM_CODE_BASE_URL and trims trailing slashes', () => {
    vi.stubEnv('SCREAM_CODE_BASE_URL', 'https://example.test/v9///');
    expect(screamCodeFeedbackUrl()).toBe('https://example.test/v9/feedback');
  });
});
...
describe('fetchSubmitFeedback', () => {
  ...
  it('preserves the scream-code- version prefix in the request body', async () => {
    const fetchMock = vi.fn(async () => new Response('', { status: 200 }));
    vi.stubGlobal('fetch', fetchMock);
    ...
    expect(sent['version']).toBe('scream-code-0.1.1');
  });
