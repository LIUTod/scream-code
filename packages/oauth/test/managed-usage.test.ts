...
describe('isManagedScreamCode', () => {
  it('matches only the scream-code managed provider', () => {
    expect(isManagedScreamCode('managed:scream-code')).toBe(true);
    expect(isManagedScreamCode('managed:scream-cli-ai')).toBe(false);
    expect(isManagedScreamCode('openai')).toBe(false);
    expect(isManagedScreamCode('')).toBe(false);
    expect(isManagedScreamCode(null)).toBe(false);
    expect(isManagedScreamCode()).toBe(false);
  });
});
...
describe('fetchManagedUsage', () => {
  ...
  it('falls back to local usage hints when the API error body is empty', async () => {
    ...
    if (result.kind !== 'error') return;
    expect(result.status).toBe(404);
    expect(result.message).toBe('Usage endpoint not available. Try Scream For Coding.');
  });
});
