...
import { SCREAM_CODE_PLATFORM } from '../src/identity';
...
describe('requestDeviceAuthorization', () => {
  it('parses a successful response', async () => {
    server.enqueue('/api/oauth/device_authorization', {
      ...
      body: {
        ...
        verification_uri: 'https://auth.scream.com/verify',
        verification_uri_complete: 'https://auth.scream.com/verify?user_code=WDJB-MJHT',
        expires_in: 600,
        interval: 5,
      },
    });
    ...
    expect(auth.verificationUri).toBe('https://auth.scream.com/verify');
    expect(auth.verificationUriComplete).toBe('https://auth.scream.com/verify?user_code=WDJB-MJHT');
  ...
  it('sends X-Msh-* device headers', async () => {
    ...
    await requestAuth();
    const recorded = server.recorded[0]!;
    expect(recorded.headers['x-msh-platform']).toBe(SCREAM_CODE_PLATFORM);
    expect(recorded.headers['x-msh-device-id']).toBe('test-device-id');
    expect(recorded.headers['x-msh-version']).toBe('0.0.0-test');
    expect(recorded.headers['user-agent'] ?? '').not.toContain('scream-code-cli');
  });
