...
async function tempHome(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'scream-telemetry-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('telemetry bootstrap', () => {
  it('matches the SCREAM_DISABLE_TELEMETRY true-value semantics', () => {
    expect(isTelemetryDisabledByEnv({ SCREAM_DISABLE_TELEMETRY: '1' })).toBe(true);
    expect(isTelemetryDisabledByEnv({ SCREAM_DISABLE_TELEMETRY: 'yes' })).toBe(true);
    expect(isTelemetryDisabledByEnv({ SCREAM_DISABLE_TELEMETRY: '0' })).toBe(false);
    expect(isTelemetryDisabledByEnv({ SCREAM_DISABLE_TELEMETRY: 'false' })).toBe(false);
  });

  it('disables the singleton without attaching a sink when opted out', async () => {
    const fetchImpl = vi.fn(async () => new Response('', { status: 200 }));
    vi.stubGlobal('fetch', fetchImpl);
    const saved = process.env['SCREAM_DISABLE_TELEMETRY'];
    try {
      process.env['SCREAM_DISABLE_TELEMETRY'] = 'true';
      initializeTelemetry({
        homeDir: await tempHome(),
        deviceId: 'dev',
        appName: 'scream-code-cli',
      ...
      if (saved === undefined) delete process.env['SCREAM_DISABLE_TELEMETRY'];
      else process.env['SCREAM_DISABLE_TELEMETRY'] = saved;
    }
