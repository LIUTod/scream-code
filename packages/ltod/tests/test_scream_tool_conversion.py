"""Tests for `_convert_tool` in the Scream chat provider.

These guard the Scream-specific schema quirk: Scream rejects tool
parameter schemas whose nested properties omit `type`, even though such
schemas are valid JSON Schema and accepted by OpenAI/Anthropic. The Scream
...
from ltod.chat_provider.scream import _convert_tool  # pyright: ignore[reportPrivateUsage]
...
def _jetbrains_like_tool() -> Tool:
    ...
    the Scream 400 — `truncateMode` has `enum` but no `type`."""
    return Tool(
...
def test_convert_tool_preserves_builtin_function_shape() -> None:
    """Scream builtin functions (names starting with `$`) don't carry
