...
class TestThinkingEffortMapping:
    ...
    (xhigh added for models after gpt-5.1-codex-max). ltod's ThinkingEffort
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("medium", "medium"),
            ("high", "high"),
            # xhigh is a valid ltod level and a valid OpenAI level — round-trip.
            ("xhigh", "xhigh"),
        ],
    )
...
class TestOpenAIStreamingErrorPropagation:
    ...
    _convert_stream_response must convert it to the correct ltod error type.
    ...
    async def test_base_api_error_becomes_connection_error(self) -> None:
        ...
        must surface as ltod APIConnectionError."""
        from ltod.contrib.chat_provider.openai_legacy import OpenAILegacyStreamedMessage
