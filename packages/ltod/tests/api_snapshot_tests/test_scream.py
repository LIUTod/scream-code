"""Snapshot tests for Scream chat provider."""
...
from ltod.message import Message, TextPart, ThinkPart, ToolCall
...
async def test_scream_message_conversion():
    with respx.mock(base_url="https://api.scream.ai") as mock:
        mock.post("/v1/chat/completions").mock(
            return_value=Response(200, json=make_chat_completion_response("scream-k2"))
        )
        provider = Scream(model="scream-k2-turbo-preview", api_key="test-key", stream=False)
...
async def test_scream_generation_kwargs():
    with respx.mock(base_url="https://api.scream.ai") as mock:
        mock.post("/v1/chat/completions").mock(
            return_value=Response(200, json=make_chat_completion_response())
        )
        provider = Scream(
            model="scream-k2-turbo-preview", api_key="test-key", stream=False
...
async def test_scream_with_thinking():
    with respx.mock(base_url="https://api.scream.ai") as mock:
        mock.post("/v1/chat/completions").mock(
            return_value=Response(200, json=make_chat_completion_response())
        )
...
async def test_scream_with_extra_body_thinking_deep_merge():
    ...
    with respx.mock(base_url="https://api.scream.ai") as mock:
        ...
        provider = (
            Scream(model="scream-k2-turbo-preview", api_key="test-key", stream=False)
            .with_thinking("high")
...
async def test_scream_with_extra_body_thinking_empty_dict_is_noop():
...
async def test_scream_with_extra_body_thinking_starts_from_empty_dict():
...
async def test_scream_with_extra_body_non_thinking_key_shallow_merge():
