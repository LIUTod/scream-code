...
from ltod.tooling import CallableTool2, ToolError, ToolOk, ToolResult, ToolReturnValue, Toolset
...
async def main():
    ...
    parser.add_argument(
        "provider",
        choices=["scream", "openai", "anthropic", "google"],
    ...
    provider: Literal["scream", "openai", "anthropic", "google"] = args.provider
    ...
    match provider:
        case "scream":
            ...
            assert api_key is not None, "Expect SCREAM_API_KEY environment variable"
            model = model or "scream-k2-turbo-preview"

            chat_provider = Scream(base_url=base_url, api_key=api_key, model=model)
        case "openai":
