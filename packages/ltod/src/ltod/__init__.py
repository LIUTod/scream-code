...
- `ltod.generate` creates a completion stream and merges streamed message parts (including
  content and tool calls) from any `ChatProvider` into a complete `Message` plus optional
  `TokenUsage`.
- `ltod.step` layers tool dispatch (`Tool`, `Toolset`, `SimpleToolset`) over `generate`,
  exposing `StepResult` with awaited tool outputs and streaming callbacks.
- Message structures and tool abstractions live under `ltod.message` and `ltod.tooling`.
...
async def main() -> None:
    client = Scream(
        base_url="https://api.scream.ai/v1",
...
from ltod._generate import GenerateResult, generate
from ltod.chat_provider import ChatProvider, ChatProviderError, StreamedMessagePart, TokenUsage
from ltod.message import Message, ToolCall
from ltod.tooling import ToolResult, ToolResultFuture, Toolset
from ltod.utils.aio import Callback
