...
from ltod.message import ContentPart, ToolCall
from ltod.utils.jsonschema import deref_json_schema
...
class DisplayBlock(BaseModel, ABC):
    ...
    Unlike `ContentPart`, ltod users may directly subclass `DisplayBlock` to define custom
...
class CallableTool(Tool, ABC):
    ...
    async def call(self, arguments: JsonType) -> ToolReturnValue:
        from ltod.tooling.error import ToolValidateError
