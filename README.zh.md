...
<p align="center">
  <img width="1007" height="890" alt="企业微信20260909-170226@2x" src="https://github.com/user-attachments/assets/d004a561-845c-4d99-a99b-10527b85ce4b" />
</p>
...
- **Goal Loop** — 目标自主驱动，独立裁判 Agent 裁决：**打回必须指出具体缺口**；缺证据可自动修（上限 3 轮），主观事项直接挂起等你决策（附书面报告）；支持轮次/Token/时间预算控制。
- **TUI 侧边栏** — 五段实时面板：工作区 Git 摘要（绿色新增/红色删除）、会话 Token 分桶（缓存命中/未命中/输出）、9 个 Agent 槽位实时状态（休息中/工作中/输出中/交谈中/返工中/求助中）、Goal 状态与裁判状态（等待中/裁定中/已裁定），以及网络 Hub（当前模型商实测响应时间 + 常用端点往返延迟；仅在侧栏可见时探测，每 30 秒一次）。
- **Wolfpack 群狼模式** — 无限制并行子 Agent（coder / explore / plan / verify / reviewer / oracle / writer / worker / designer）。
- **子代理协作** — 把任务交给专职子代理，运行中也能随时指挥：**受阻时会主动找主 Agent 商量而不是瞎猜**，需要时请求换擅长的人接手；支持发送即时调整指令或补充说明、要求 JSON 结构化输出；还能限制为「只读 / 仅编辑 / 可执行命令」安全模式，防止越权。
- **Bot 无人值守模式（`/bot`）** — 你离开也能干活：可逆操作（读、工作区内改代码、只读命令）自动执行；不可逆操作（推代码 / 发布 / 删除 / 敏感文件 / 未分类命令）自动拒绝并排队等你审——拒绝理由会告知模型，避免盲目重试。
...
- **MCP / Skill / 130+ 模型商** — 全部可 DIY 配置；内置浏览器自动化与 macOS 桌面自动化。
...
| `/yes`（`/yolo`） | 切换自动批准模式 |
| `/ask` | 切换只读问答模式 |
| `/bot` | 无人值守 Bot 模式：可逆操作自动执行，不可逆的停下等你审 |
...
| `/blockrows` | 调整活动块行数（折叠高度 / 工具结果行数 / 思考行数） |
| `/codebg` | 开关代码块背景色面板 |
| `/mermaid` | 选择 Mermaid 块的渲染方式：用制表符边框画成图 · 照样画但边框改用 ASCII 字符（终端把制表符画成两格宽时用） · 不画、保留源码 |
| `/snaptimer`（`/timer`） | 开关会话快照计时器（每回合耗时标记） |
| `/sidebar` | 打开/关闭侧边面板（`Ctrl+X` 快速开关） |
| `/tasks`（`/task`） | 浏览后台任务 |
| `/cron` | 定时任务：列表、新建（选时间 + 输入任务）、删除 |
