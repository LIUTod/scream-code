...
- **TUI 侧边栏** — 五段实时面板：工作区 Git 摘要（绿色新增/红色删除）、会话 Token 分桶（缓存命中/未命中/输出）、8 个 Agent 槽位实时状态（休息中/工作中/输出中/交谈中/返工中/求助中）、Goal 状态与裁判状态（等待中/裁定中/已裁定），以及网络 Hub（当前模型商实测响应时间 + 常用端点往返延迟；仅在侧栏可见时探测，每 30 秒一次）。
