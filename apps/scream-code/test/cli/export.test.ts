...
describe('scream export', () => {
  ...
  it('initializes and flushes telemetry around default export tracking', async () => {
    ...
    await program.parseAsync(['node', 'scream', 'export', 'ses_telemetry', '--output', output], {
      from: 'node',
    });
  ...
  it('passes enabled false when default export config disables telemetry', async () => {
    ...
    await program.parseAsync(['node', 'scream', 'export', 'ses_disabled', '--output', output], {
      from: 'node',
    });
  ...
  it('tracks first launch around default export telemetry before harness construction can create the device id', async () => {
    ...
    await program.parseAsync(['node', 'scream', 'export', 'ses_first_launch', '--output', output], {
      from: 'node',
    });
    ...
    expect(mocks.screamHarnessConstructor).toHaveBeenCalledWith(
      expect.objectContaining({ homeDir: '/tmp/scream-export-home' }),
    );
