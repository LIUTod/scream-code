import type { createScreamDeviceId as createScreamDeviceIdFn } from '@scream-code/config';
...
vi.mock('@scream-code/config', async () => {
  const actual = await vi.importActual<typeof import('@scream-code/config')>(
    '@scream-code/config',
  );
  return {
    ...actual,
