...
export function createControlModule(ctx: ClientContext): ControlModule {
  ...
  async function fetchSessionStatus(): Promise<void> {
    ...
    try {
      const res = await fetch(`${API_BASE}/sessions/${token.sessionId}/status`);
      if (!current(token) || request !== statusRequest || !res.ok) return;
  ...
  async function fetchSessionUsage(): Promise<void> {
    ...
    try {
      const res = await fetch(`${API_BASE}/sessions/${token.sessionId}/usage`);
      if (!current(token) || request !== usageRequest || !res.ok) return;
  ...
  async function fetchSessionContext(): Promise<void> {
    ...
    try {
      const res = await fetch(`${API_BASE}/sessions/${token.sessionId}/context`);
      if (!current(token) || request !== contextRequest || !res.ok) return;
  ...
  async function fetchSessionPlan(): Promise<void> {
    ...
    try {
      const res = await fetch(`${API_BASE}/sessions/${token.sessionId}/plan`);
      if (!current(token) || request !== planRequest || !res.ok) return;
  ...
  async function clearPlan(): Promise<boolean> {
    ...
    // Invalidate an in-flight plan read: otherwise an older GET can resolve
    // after this successful clear and put the just-deleted plan back in the UI.
    const request = ++planRequest;
    try {
      const res = await fetch(`${API_BASE}/sessions/${token.sessionId}/plan/clear`, { method: 'POST' });
      if (!current(token) || request !== planRequest || !res.ok) return false;
  ...
  async function postSessionAction(path: string, body?: Record<string, unknown>): Promise<boolean> {
    ...
    try {
      const res = await fetch(`${API_BASE}/sessions/${token.sessionId}/${path}`, {
        method: 'POST',
