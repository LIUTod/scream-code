...
- **TUI Sidebar** — five live panels: workspace Git diff summary (green adds / red deletes), per-session token accounting (cache-hit / cache-miss / output), 8 sub-agent slots with real-time states (idle / working / outputting / messaging / reworking / asking), Goal status with judge state (awaiting / judging / adjudicated), and a Network Hub — the measured provider response time plus round trips to a few well-known endpoints, probed only while the sidebar is visible, once a minute.
