...
WHEEL_DIR="${SCREAM_CLI_WHEEL_DIR:-${REPO_ROOT}/dist/accuracy_smoke}"
JOBS_DIR="${HARBOR_JOBS_DIR:-${REPO_ROOT}/jobs}"
MODEL="${HARBOR_MODEL:-scream/scream-for-coding}"
AGENT_IMPORT_PATH="${HARBOR_AGENT_IMPORT_PATH:-tests_ai.accuracy_smoke.local_scream_cli_agent:LocalScreamCli}"
...
if [ -z "${SCREAM_API_KEY:-}" ]; then
  echo "Missing API key. Set SCREAM_API_KEY first." >&2
  echo 'Example: export SCREAM_API_KEY="your_api_key"' >&2
  exit 1
fi

mkdir -p "${WHEEL_DIR}"
echo "Building local scream-code wheel into ${WHEEL_DIR}"
...
UV_PYTHON="${UV_PYTHON}" UV_PYTHON_INSTALL_MIRROR="${UV_PYTHON_INSTALL_MIRROR}" \
  uv build --out-dir "${WHEEL_DIR}" >/dev/null
SCREAM_CLI_WHEEL_PATH="$(ls -t "${WHEEL_DIR}"/*.whl | head -n 1)"

echo "Using local scream-code wheel: ${SCREAM_CLI_WHEEL_PATH}"
...
while IFS= read -r task || [ -n "${task}" ]; do
  ...
  SCREAM_CLI_WHEEL_PATH="${SCREAM_CLI_WHEEL_PATH}" \
