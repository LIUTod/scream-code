...
This directory hosts a benchmark-backed accuracy smoke layer for `scream-cli`.
...
Harbor's custom local `scream-cli` agent reads credentials from environment variables.
Set the following before running smoke tasks:
...
```bash
export SCREAM_API_KEY="your_api_key"
bash tests_ai/accuracy_smoke/scripts/run_smoke.sh
```
...
- `HARBOR_MODEL` (default: `scream/scream-for-coding`)

## Use current scream-cli source (not release build)

`run_smoke.sh` defaults to evaluating the current repository commit by using a
custom Harbor agent import path:

- `tests_ai.accuracy_smoke.local_scream_cli_agent:LocalScreamCli`
...
- `SCREAM_CLI_WHEEL_DIR` (default: `dist/accuracy_smoke`)
...
```bash
SCREAM_CLI_WHEEL_DIR=/tmp/scream-wheel-cache \
  bash tests_ai/accuracy_smoke/scripts/run_smoke.sh
```
