...
from scream.soul import LLMNotSet, wire_send
from scream.soul.dynamic_injection import normalize_history
from scream.soul.message import system_reminder
from scream.utils.logging import logger
from scream.wire.types import BtwBegin, BtwEnd, TextPart
...
def _build_btw_context(
    ...
    as ``ScreamSoul._step`` so the LLM provider can reuse the prompt cache.
    """
...
async def execute_side_question(
    ...
    Args:
        soul: The ScreamSoul instance (for context and chat_provider access).
...
async def run_side_question(soul: ScreamSoul, question: str) -> None:
