...
from ltod.tooling import Toolset

from scream.agentspec import load_agent_spec
from scream.approval_runtime import ApprovalRuntime
from scream.background import BackgroundTaskManager
...
from scream.subagents.models import AgentTypeDefinition, ToolPolicy
from scream.subagents.registry import LaborMarket
from scream.subagents.store import SubagentStore
from scream.utils.environment import Environment
from scream.utils.logging import logger
from scream.utils.path import find_project_root, is_within_directory, list_directory
from scream.wire.root_hub import RootWireHub
...
async def load_agents_md(work_dir: KaosPath) -> str | None:
    ...
    1. ``.scream/AGENTS.md``  — project-local scream config (highest priority)
    2. ``AGENTS.md``        — standard location
    3. ``agents.md``        — lowercase variant (mutually exclusive with 2)

    Within a single directory, ``.scream/AGENTS.md`` and ``AGENTS.md``/``agents.md``
    are **both** loaded (with ``.scream/`` first), but ``AGENTS.md`` and ``agents.md``
    ...
    for d in dirs:
        # .scream/AGENTS.md is always checked independently (can coexist with root-level file)
        scream_path = d / ".scream" / "AGENTS.md"
        ...
        if await scream_path.is_file():
            candidates.append(scream_path)
...
class Runtime:
    ...
    """HookEngine instance, set by ScreamCLI after soul creation."""
    ...
    async def create(
        ...
        # 创建 PermissionEngine，加载用户自定义规则和会话覆盖
        permission_engine = PermissionEngine()
        if config.permission_rules:
            try:
                user_engine = PermissionEngine.from_dict_list(config.permission_rules)
                permission_engine = user_engine
            except Exception as exc:
                logger.warning("Failed to load permission rules from config: {exc}", exc=exc)
        if session.state.approval.permission_overrides:
            try:
                for rule_data in session.state.approval.permission_overrides:
                    from scream.permission import PermissionAction, PermissionRule

                    permission_engine.add_session_rule(
                        PermissionRule(
                            tool_pattern=rule_data.get("tool_pattern", "*"),
                            action=PermissionAction(rule_data.get("action", "ask")),
                            path_pattern=rule_data.get("path_pattern"),
                            description=rule_data.get("description", ""),
                        )
                    )
            except Exception as exc:
                logger.warning("Failed to load permission overrides from session: {exc}", exc=exc)
        ...
        return Runtime(
            ...
            session=session,
            builtin_args=BuiltinSystemPromptArgs(
                SCREAM_NOW=datetime.now().astimezone().isoformat(),
                SCREAM_WORK_DIR=session.work_dir,
                SCREAM_WORK_DIR_LS=ls_output,
                SCREAM_AGENTS_MD=agents_md or "",
                SCREAM_SKILLS=skills_formatted or "No skills found.",
                SCREAM_ADDITIONAL_DIRS_INFO=additional_dirs_info,
                SCREAM_OS=environment.os_kind,
                SCREAM_SHELL=f"{environment.shell_name} (`{environment.shell_path}`)",
            ),
            denwa_renji=DenwaRenji(),
            approval=Approval(state=approval_state, permission_engine=permission_engine),
            labor_market=LaborMarket(),
            environment=environment,
...
async def load_agent(
    ...
    *,
    ...
    Raises:
        ...
        SystemPromptTemplateError(ScreamCLIException, ValueError): When the system prompt template
            is invalid.
        ...
        MCPRuntimeError(ScreamCLIException, RuntimeError): When any MCP server cannot be connected.
    """
    ...
    # Load plugin tools
    from scream.plugin.manager import get_plugins_dir
    from scream.plugin.tool import load_plugin_tools
