...
from ltod.tooling.error import (
    ToolNotFoundError,
    ToolParseError,
    ToolRuntimeError,
)
from ltod.tooling.mcp import convert_mcp_content
from ltod.utils.typing import JsonType

from scream import logger
from scream.exception import InvalidToolError, MCPRuntimeError
...
if TYPE_CHECKING:
    ...
    def type_check(scream_toolset: ScreamToolset):
        _: Toolset = scream_toolset
...
def _append_reminder_to_return_value(return_value: Any) -> Any:
    ...
    from ltod.tooling import ToolReturnValue
...
class ScreamToolset:
    ...
    def handle(self, tool_call: ToolCall) -> HandleResult:
        ...
        try:
            ...
            async def _call():
                    ...
                    if result.action == "block":
                        return ToolResult(
                            ...
                            return_value=ToolError(
                                message=result.reason or "被 PreToolUse 钩子阻止",
                                brief="钩子阻止",
                            ),
                        )
                ...
                from scream.telemetry import track as _track_tool_call
    ...
    def load_tools(self, tool_paths: list[str], dependencies: dict[type[Any], Any]) -> None:
        """
        Load tools from paths like `scream.tools.shell:Shell`.
    ...
    async def load_mcp_tools(
        ...
        from scream.ui.shell.prompt import toast
        ...
        for mcp_config in mcp_configs:
            ...
            for server_name, server_config in mcp_config.mcpServers.items():
                if isinstance(server_config, RemoteMCPServer) and server_config.auth == "oauth":
                    logger.warning(
                        "Skipping OAuth MCP server '{server_name}': OAuth is not supported.",
                        server_name=server_name,
                    )
                    continue
...
class WireExternalTool(CallableTool):
    ...
    async def __call__(self, *args: Any, **kwargs: Any) -> ToolReturnValue:
        ...
        from scream.soul import get_wire_or_none
...
def convert_mcp_tool_result(result: CallToolResult) -> ToolReturnValue:
    """Convert MCP tool result to ltod tool return value.
