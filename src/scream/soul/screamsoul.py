...
from scream.skill import Skill, read_skill_text
from scream.skill.flow import Flow, FlowEdge, FlowNode, parse_choice
from scream.soul import (
    LLMNotSet,
    LLMNotSupported,
    MaxStepsReached,
    Soul,
...
from scream.soul.compaction import (
    CompactionResult,
    SimpleCompaction,
...
from scream.soul.message import check_message, system, system_reminder, tool_result_to_message
from scream.soul.slash import registry as soul_slash_registry
from scream.soul.toolset import ScreamToolset
from scream.tools.dmail import NAME as SendDMail_NAME
from scream.tools.utils import ToolRejectedError
from scream.utils.logging import logger
from scream.utils.slashcmd import SlashCommand, parse_slash_command_call
from scream.wire.file import WireFile
...
if TYPE_CHECKING:
    ...
    def type_check(soul: ScreamSoul):
        _: Soul = soul
...
class ScreamSoul:
    ...
    def __init__(
        ...
        *,
        ...
        if self._plan_session_id is not None and self._runtime.session.state.plan_slug is not None:
            from scream.tools.plan.heroes import seed_slug_cache
        ...
        self._injection_providers: list[DynamicInjectionProvider] = [
            ...
            *(
            ...
            MemoryInjectionProvider(self._memory_manager),
        ]
    ...
    def set_hook_engine(self, engine: HookEngine) -> None:
        ...
        if isinstance(self._agent.toolset, ScreamToolset):
    ...
    def _bind_plan_mode_tools(self) -> None:
        """Bind plan mode state to tools that support it."""
        if not isinstance(self._agent.toolset, ScreamToolset):
            return
        ...
        from scream.tools.file.write import WriteFile
        ...
        from scream.tools.file.replace import StrReplaceFile
        ...
        from scream.tools.plan import ExitPlanMode
        ...
        from scream.tools.plan.enter import EnterPlanMode
        ...
        from scream.tools.ask_user import AskUserQuestion
    ...
    def _ensure_plan_session_id(self) -> None:
        ...
        if self._plan_session_id is None:
            ...
            from scream.tools.plan.heroes import get_or_create_slug
    ...
    def get_plan_file_path(self) -> Path | None:
        ...
        if self._plan_session_id is None:
            return None
        from scream.tools.plan.heroes import get_plan_file_path
    ...
    def read_current_plan(self) -> str | None:
        ...
        if self._plan_session_id is None:
            return None
        from scream.tools.plan.heroes import read_plan_file
    ...
    async def run(
        ...
        *,
        ...
        try:
            ...
            from scream.soul.toolset import set_session_id

            set_session_id(self._runtime.session.id)

            from scream.hooks import events
            ...
            wire_send(TurnBegin(user_input=user_input))
            turn_started = True
            from scream.telemetry import track as _track_telemetry
            ...
            if not command_call:
                ...
                if session.state.custom_title is None:
                    ...
                    if title:
                        from scream.session_state import (
                            load_session_state,
                            save_session_state,
                        )
    ...
    async def _agent_loop(self) -> TurnOutcome:
        ...
        if isinstance(self._agent.toolset, ScreamToolset):
            ...
            try:
                ...
                if loading:
                    from scream.telemetry import track as _track_mcp
        ...
        while True:
            ...
            try:
                ...
                _scream_ctx = getattr(e, "_scream_api_error_context", None)
                if _scream_ctx is not None:
                    for key in ("model", "duration_ms", "input_tokens"):
                        if key in _scream_ctx:
                            track_kwargs[key] = _scream_ctx[key]
                track("api_error", **track_kwargs)

                # --- StopFailure hook ---
                from scream.hooks import events as _hook_events
    ...
    async def _step(self) -> StepOutcome | None:
        ...
        Sub-lifecycle (2e.x):
            ...
            2e.4. LLM call with retry    - ltod.step + tenacity retry + recovery.
               2e.4.1. Toolset begin_step- reset per-step dedup state.
               2e.4.2. ltod.step       - actual LLM call (may be interrupted).
        ...
        async def _ltod_step_with_retry() -> StepResult:
        ...
        t0 = time.monotonic()
        try:
            result = await _ltod_step_with_retry()
            ...
            _step_exc._scream_api_error_context = _ctx  # type: ignore[attr-defined]
            raise
    ...
    def _is_retryable_error(exception: BaseException) -> bool:
        if isinstance(exception, (APIConnectionError, APITimeoutError)):
            return not bool(getattr(exception, "_scream_recovery_exhausted", False))
        ...
        *,
        ...
        try:
            ...
            if error.status_code != 401 or _auth_retried:
                raise
            # No OAuth token refresh — just retry once on 401
            logger.warning(
                "Received 401 during {name}, retrying once",
                name=name,
            )
            return await self._run_with_connection_recovery(
                name,
            ...
            if _connection_retried:
                ...
                error._scream_recovery_exhausted = True  # type: ignore[attr-defined]
                raise
...
class FlowRunner:
    ...
    async def run(self, soul: ScreamSoul, args: str) -> None:
        if args.strip():
