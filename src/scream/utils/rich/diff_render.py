...
from scream.tools.display import DiffDisplayBlock
from scream.ui.theme import get_diff_colors
...
def _make_highlighter(path: str) -> ScreamSyntax:
    """Create a ScreamSyntax instance for highlighting code by file extension."""
    ext = path.rsplit(".", 1)[-1] if "." in path else ""
    return ScreamSyntax("", ext if ext else "text")


def _highlight(highlighter: ScreamSyntax, code: str) -> Text:
...
def _highlight_hunk(highlighter: ScreamSyntax, hunk: list[DiffLine]) -> None:
