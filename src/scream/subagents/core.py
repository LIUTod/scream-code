...
from scream.subagents.models import AgentLaunchSpec, AgentTypeDefinition
...
async def prepare_soul(
    ...
    if spec.type_def.name == "explore" and not spec.resumed:
        from scream.subagents.git_context import collect_git_context

        git_ctx = await collect_git_context(runtime.builtin_args.SCREAM_WORK_DIR)
        if git_ctx:
            prompt = f"{git_ctx}\n\n{prompt}"
