...
import aiohttp
from ltod.tooling import CallableTool2, ToolError, ToolReturnValue
...
from scream.utils.sensitive import is_sensitive_file, sensitive_file_warning
...
RG_VERSION = "15.0.0"
RG_BASE_URL = "http://cdn.example.com/binaries/scream/rg"
...
def _find_existing_rg(bin_name: str) -> Path | None:
    ...
    assert scream.__file__ is not None
    local_dep = Path(scream.__file__).parent / "deps" / "bin" / bin_name
...
async def _download_and_install_rg(bin_name: str) -> Path:
    ...
    async with new_client_session(timeout=download_timeout) as session:
        with tempfile.TemporaryDirectory(prefix="scream-rg-") as tmpdir:
