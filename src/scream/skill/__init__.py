...
from scream.skill.flow import Flow, FlowError
from scream.skill.flow.d2 import parse_d2_flowchart
from scream.skill.flow.mermaid import parse_mermaid_flowchart
from scream.utils.frontmatter import parse_frontmatter
...
- ``builtin``: bundled with scream-code
- ``user``: from the user's home (``~/.scream/skills``, ``~/.agents/skills``, ...)
- ``project``: from the current project's working directory
  (``<work_dir>/.scream/skills``, ``<work_dir>/.agents/skills``, ...)
...
def get_builtin_skills_dir() -> Path:
    ...
    if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
        ...
        return Path(meipass) / "scream" / "skills"
...
def _get_user_brand_skills_dir_candidates() -> tuple[KaosPath, ...]:
    ...
    Brand group: ``~/.scream/skills``
    """
    return (
        KaosPath.home() / ".scream" / "skills",
    )
...
async def find_user_skills_dirs(
    *,
    ...
    directories are included (priority order: scream only).
    """
    dirs: list[KaosPath] = []
    if merge_brands:
...
async def find_project_skills_dirs(
    ...
    *,
    ...
    of ``work_dir``), so launching scream-code from a subdirectory — for example
    ...
    directories are included (priority order: scream only).
    """
    from scream.utils.path import find_project_root
...
def _first_meaningful_line(content: str) -> str | None:
    ...
    :mod:`scream.utils.frontmatter`). A malformed frontmatter opener that
    ...
    """
    from scream.utils.frontmatter import strip_frontmatter
