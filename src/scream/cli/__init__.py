...
cli = typer.Typer(
    ...
    epilog="""\b\
Documentation:        https://github.com/LIUTod/scream-code""",
    add_completion=False,
    context_settings={"help_option_names": ["-h", "--help"]},
    help="Scream, your next CLI agent.",
)
...
def _version_callback(value: bool) -> None:
    if value:
        ...
        typer.echo(f"scream, version {get_version()}")
        raise typer.Exit()
...
def scream(
    ...
    config_file: Annotated[
        ...
        typer.Option(
            ...
            dir_okay=False,
            readable=True,
            help="Config TOML/JSON file to load. Default: ~/.scream/config.toml.",
        ),
    ] = None,
    ...
    acp_mode: Annotated[
        ...
        typer.Option(
            "--acp",
            help="(Deprecated, use `scream acp` instead) Run as ACP server.",
        ),
    ] = False,
    wire_mode: Annotated[
        bool,
    ...
    """Scream, your next CLI agent."""
    import asyncio
    import contextlib
    import json

    from scream.utils.proctitle import init_process_name

    init_process_name("Scream Code")
    ...
    from scream.agentspec import DEFAULT_AGENT_FILE, OKABE_AGENT_FILE
    from scream.app import ScreamCLI, enable_logging
    from scream.config import Config, load_config_from_string
    from scream.exception import ConfigError
    from scream.hooks import events as hook_events
    from scream.metadata import load_metadata, save_metadata
    from scream.session import Session
    from scream.ui.shell.startup import ShellStartupProgress
    ...
    # Redirection is installed later, right before ScreamCLI.create(), so that
    ...
    async def _run(session_id: str | None, prefill_text: str | None = None) -> tuple[Session, int]:
        ...
        try:
            ...
            if local_add_dirs:
                from scream.utils.path import is_within_directory
            ...
            # Redirect stderr *before* ScreamCLI.create() so that MCP server
            ...
            instance = await ScreamCLI.create(
                session,
                config=config,
    ...
    def _print_resume_hint(session: Session) -> None:
        ...
        if not session.is_empty():
            _emit_fatal_error(f"\nTo resume this session: scream -r {session.id}")
    ...
    async def _reload_loop(session_id: str | None) -> int:
        """Run the main loop, handling Reload.

        Returns:
            exit_code (0 = success, 1 = failure, 75 = retryable).
        """
    ...
    if _picker_mode:
        ...
        from scream.utils.datetime import format_relative_time
    ...
    try:
        exit_code = asyncio.run(_reload_loop(session_id))
        ...
        if debug:
            ...
            log_path = get_share_dir() / "logs" / "scream.log"
            # In non-debug mode, print a concise error and point users to logs.
            _emit_fatal_error(
                f"{exc}\n"
                f"See logs: {log_path}\n"
                "Run with --debug for full traceback, or run scream export to share diagnostics."
            )
...
def login(
    ...
    """Login is not supported. Configure providers via config file or environment variables."""
    typer.echo(
        "OAuth login is not supported. "
        "Configure your AI provider via the config file or environment variables.",
        err=True,
    )
...
def logout(
    ...
    typer.echo("OAuth logout is not supported.", err=True)
    raise typer.Exit(code=1)
...
def term(
    ...
    """Run Toad TUI backed by Scream Code CLI ACP server."""
...
@cli.command()
def acp():
    """Run Scream Code CLI ACP server."""
    from scream.acp import acp_main
...
def background_task_worker(
    ...
    from scream.background import run_background_task_worker
    from scream.utils.proctitle import set_process_title

    set_process_title("scream-code-bg-worker")
...
if __name__ == "__main__":
    ...
    if "scream.cli" not in sys.modules:
        sys.modules["scream.cli"] = sys.modules[__name__]
