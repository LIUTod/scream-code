...
from scream.plugin import PluginError
...
def _resolve_source(target: str) -> tuple[Path, Path | None]:
    ...
    if parsed.scheme in ("http", "https") and parsed.path.lower().endswith(".zip"):
        ...
        tmp = Path(tempfile.mkdtemp(prefix="scream-plugin-"))
    ...
    if target.startswith(("https://", "git@", "http://")) and (
        ...
        if available:
            ...
            typer.echo(
                ...
                f"Available plugins:\n{names}\n"
                f"Use: scream plugin install <url>/<plugin-name>",
                err=True,
            )
        else:
            typer.echo(
...
def install_cmd(
    ...
    from scream.config import load_config
    from scream.constant import VERSION
    from scream.plugin.manager import get_plugins_dir, install_plugin
    ...
    try:
        ...
        from scream.llm import augment_provider_with_env_vars
        from scream.plugin.manager import collect_host_values
        ...
        if not host_values.get("api_key"):
            typer.echo(
                ...
                "Configure a provider in the config file first.",
                err=True,
            )
...
def list_cmd() -> None:
    ...
    from scream.plugin.manager import get_plugins_dir, list_plugins
...
def remove_cmd(
    ...
    from scream.plugin.manager import get_plugins_dir, remove_plugin
...
def info_cmd(
    ...
    """Show plugin details."""
    from scream.plugin import parse_plugin_json
