# ACP Integration Notes (scream-cli)
...
- **Single-session server**: `ScreamCLI.run_acp()` uses `ACP` -> `ACPServerSingleSession`.
  - Code: `src/scream/app.py`, `src/scream/ui/acp/__init__.py`.
  - Used when running CLI with `--acp` UI mode.
- **Multi-session server**: `acp_main()` runs `ACPServer` with `use_unstable_protocol=True`.
  - Code: `src/scream/acp/__init__.py`, `src/scream/acp/server.py`.
  - Exposed via the `scream acp` command in `src/scream/cli/__init__.py`.
...
- `session/new`
  - Multi-session: creates a persisted `Session`, builds `ScreamCLI`, stores `ACPSession`.
...
- `session/load`
  - Multi-session only: loads by `Session.find`, then builds `ScreamCLI` and `ACPSession`.
...
- `session/set_mode` and `session/set_model` (no multi-mode/model switching in scream-cli).
