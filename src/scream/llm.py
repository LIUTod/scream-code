...
if TYPE_CHECKING:
    from scream.config import Config, LLMModel, LLMProvider
...
def model_display_name(model_name: str | None, model: LLMModel | None = None) -> str:
    ...
    if not model_name:
        return ""
    if model_name in ("scream-for-coding", "scream-code"):
        return "scream-for-coding"
    return model_name
...
def augment_provider_with_env_vars(provider: LLMProvider, model: LLMModel) -> dict[str, str]:
    ...
    match provider.type:
        case "scream":
            if base_url := os.getenv("SCREAM_BASE_URL"):
                provider.base_url = base_url
                applied["SCREAM_BASE_URL"] = base_url
            if api_key := os.getenv("SCREAM_API_KEY"):
                provider.api_key = SecretStr(api_key)
                applied["SCREAM_API_KEY"] = "******"
            if model_name := os.getenv("SCREAM_MODEL_NAME"):
                model.model = model_name
                applied["SCREAM_MODEL_NAME"] = model_name
            if max_context_size := os.getenv("SCREAM_MODEL_MAX_CONTEXT_SIZE"):
                model.max_context_size = int(max_context_size)
                applied["SCREAM_MODEL_MAX_CONTEXT_SIZE"] = max_context_size
            if capabilities := os.getenv("SCREAM_MODEL_CAPABILITIES"):
                ...
                applied["SCREAM_MODEL_CAPABILITIES"] = capabilities
        case "anthropic":
            ...
            if max_context_size := os.getenv("ANTHROPIC_MODEL_MAX_CONTEXT_SIZE"):
                model.max_context_size = int(max_context_size)
                applied["ANTHROPIC_MODEL_MAX_CONTEXT_SIZE"] = max_context_size
...
def _scream_default_headers(provider: LLMProvider) -> dict[str, str]:
    ...
    *,
    ...
    match provider.type:
        case "scream":
            ...
            chat_provider = Scream(
                model=model.model,
                base_url=provider.base_url,
                api_key=resolved_api_key,
                default_headers=_scream_default_headers(provider),
            )

            gen_kwargs: Scream.GenerationKwargs = {}
            if session_id:
                gen_kwargs["prompt_cache_key"] = session_id
            if temperature := os.getenv("SCREAM_MODEL_TEMPERATURE"):
                gen_kwargs["temperature"] = float(temperature)
            if top_p := os.getenv("SCREAM_MODEL_TOP_P"):
                gen_kwargs["top_p"] = float(top_p)
            if max_tokens := os.getenv("SCREAM_MODEL_MAX_TOKENS"):
        ...
        case "_scripted_echo":
            from ltod.chat_provider.echo import ScriptedEchoChatProvider

            if provider.env:
                os.environ.update(provider.env)
            scripts = _load_scripted_echo_scripts()
            trace_value = os.getenv("SCREAM_SCRIPTED_ECHO_TRACE", "")
    ...
    # Apply Scream-specific ``thinking.keep`` (preserved thinking) only when
    ...
    if thinking_on and provider.type == "scream":
        from ltod.chat_provider.scream import Scream

        if isinstance(chat_provider, Scream) and (
            thinking_keep := os.getenv("SCREAM_MODEL_THINKING_KEEP")
        ):
...
def derive_model_capabilities(model: LLMModel) -> set[ModelCapability]:
    ...
    elif model.model in {"scream-for-coding", "scream-code"}:
        capabilities.update(("thinking", "image_in", "video_in"))
    return capabilities


def _load_scripted_echo_scripts() -> list[str]:
    script_path = os.getenv("SCREAM_SCRIPTED_ECHO_SCRIPTS")
    if not script_path:
        raise ValueError("SCREAM_SCRIPTED_ECHO_SCRIPTS is required for _scripted_echo.")
