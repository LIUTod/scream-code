...
def get_version() -> str:
    ...
    for dist_name in ("scream-code", "scream"):
        try:
...
def get_user_agent() -> str:
    return f"ScreamCLI/{get_version()}"
...
def get_build_sha() -> str:
    ...
    Priority:
    1. SCREAM_BUILD_SHA environment variable (dev / CI override)
    ...
    """
    if build_id := os.environ.get("SCREAM_BUILD_SHA", "").strip():
        return build_id

    try:
        from scream._build_info import BUILD_SHA  # type: ignore[reportMissingImports]
