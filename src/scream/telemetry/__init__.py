"""
Telemetry event tracking for scream-cli.

This module has NO dependencies on other scream modules to avoid import cycles.
...
Usage:
    from scream.telemetry import track, set_context, attach_sink
...
def attach_sink(sink: EventSink) -> None:
    ...
    Multi-session ACP mode calls ``ScreamCLI.create()`` per session, which
