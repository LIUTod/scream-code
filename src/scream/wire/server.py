...
import pydantic
from ltod.chat_provider import APIStatusError, ChatProviderError
from ltod.tooling import ToolError, ToolResult
from ltod.utils.typing import JsonType
...
from scream.soul.toolset import ScreamToolset, WireExternalTool
from scream.utils.aioqueue import Queue, QueueShutDown
...
class WireServer:
    ...
    async def serve(self) -> None:
        ...
        if isinstance(self._soul, ScreamSoul) and self._soul.runtime.root_wire_hub is not None:
    ...
    async def _shutdown(self) -> None:
        ...
        if (
            isinstance(self._soul, ScreamSoul)
    ...
    async def _handle_initialize(
        ...
        from scream.constant import NAME, VERSION
        from scream.hooks.config import HOOK_EVENT_TYPES
        from scream.hooks.engine import WireHookHandle, WireHookSubscription
        from scream.soul import wire_send
        from scream.wire.protocol import WIRE_PROTOCOL_VERSION
        from scream.wire.types import HookResolved, HookTriggered
    ...
    def _sync_ask_user_tool_visibility(self, toolset: ScreamToolset) -> None:
        """Hide or unhide the AskUserQuestion tool based on client capabilities."""
        from scream.tools.ask_user import NAME as ASK_USER_TOOL_NAME
    ...
    def _sync_plan_mode_tool_visibility(self, toolset: ScreamToolset) -> None:
        """Hide or unhide plan mode tools based on client capabilities."""
        from scream.tools.plan import NAME as EXIT_PLAN_MODE_TOOL_NAME
        from scream.tools.plan.enter import NAME as ENTER_PLAN_MODE_TOOL_NAME
    ...
    def _apply_wire_client_info(self, client: ClientInfo | None) -> None:
        if client is not None:
            from scream.telemetry import set_client_info

            set_client_info(name=client.name, version=client.version)

        if not isinstance(self._soul, ScreamSoul):
            return
        ...
        if isinstance(llm.chat_provider, Scream):
            screament = llm.chat_provider.client
            headers = dict(screament._custom_headers)  # pyright: ignore[reportPrivateUsage]
            headers["User-Agent"] = f"{USER_AGENT}{ua_suffix}"
            screament._custom_headers = headers  # pyright: ignore[reportPrivateUsage]

    def _track_session_started(self, client: ClientInfo | None) -> None:
        if not isinstance(self._soul, ScreamSoul):
            return

        from scream.telemetry import track_session_started_once
    ...
    async def _handle_prompt(
        ...
        runtime = self._soul.runtime if isinstance(self._soul, ScreamSoul) else None
        try:
            await run_soul(
                self._soul,
    ...
    async def _handle_steer(
        ...
        if not isinstance(self._soul, ScreamSoul) or not self._is_streaming:
            return JSONRPCErrorResponse(
                id=msg.id,
                error=JSONRPCErrorObject(
                    code=ErrorCodes.INVALID_STATE,
    ...
    async def _handle_replay(
        ...
        wire_file = self._soul.wire_file if isinstance(self._soul, ScreamSoul) else None
