...
from ltod.chat_provider import (
    APIConnectionError,
...
from scream.soul.screamsoul import FLOW_COMMAND_PREFIX, ScreamSoul
from scream.ui.shell import update as _update_mod
from scream.ui.shell.console import console
from scream.ui.shell.echo import render_user_echo_text
from scream.ui.shell.mcp_status import render_mcp_prompt
from scream.ui.shell.prompt import (
    BgTaskCounts,
    CustomPromptSession,
    CwdLostError,
    PromptMode,
...
from scream.ui.shell.replay import replay_recent_history
from scream.ui.shell.slash import SKILL_COMMAND_PREFIX, shell_mode_registry
from scream.ui.shell.slash import registry as shell_slash_registry
from scream.ui.shell.update import LATEST_VERSION_FILE, UpdateResult, do_update, semver_tuple
from scream.ui.shell.visualize import (
    ApprovalPromptDelegate,
    visualize,
)
from scream.utils.aioqueue import QueueShutDown
from scream.utils.envvar import get_env_bool
from scream.utils.logging import open_original_stderr
from scream.utils.signals import install_sigint_handler
from scream.utils.slashcmd import SlashCommand, SlashCommandCall, parse_slash_command_call
from scream.utils.subprocess_env import get_clean_env
from scream.utils.term import ensure_new_line, ensure_tty_sane
...
class Shell:
    def __init__(
        ...
        needs_first_time_config: bool = False,
    ):
        self.soul = soul
        self._welcome_info = list(welcome_info or [])
        self._prefill_text = prefill_text
        self._needs_first_time_config = needs_first_time_config
    ...
    def _print_cwd_lost_crash(self) -> None:
        ...
        runtime = self.soul.runtime if isinstance(self.soul, ScreamSoul) else None
    ...
    async def run(self, command: str | None = None) -> bool:
        ...
        if isinstance(self.soul, ScreamSoul):
            from scream.ui.theme import set_active_theme
        ...
        if get_env_bool("SCREAM_CLI_NO_AUTO_UPDATE"):
            logger.info("Auto-update disabled by SCREAM_CLI_NO_AUTO_UPDATE environment variable")
        else:
            self._start_background_task(self._auto_update())

        _print_welcome_info(self.soul.name or "Scream Code CLI", self._welcome_info)

        # Auto-launch config wizard on first run if no models are configured
        if self._needs_first_time_config and isinstance(self.soul, ScreamSoul):
            console.print("[yellow]未检测到模型配置，正在启动配置向导...[/yellow]")
            ...
            # but warn that no model is configured
            if not self.soul.runtime.config.models:
                console.print(
                    "[yellow]配置未保存。你可以随时输入 /config 来配置模型。[/yellow]"
                )

        # Start telemetry periodic flush and disk retry
        from scream.telemetry import get_sink
        ...
        def _mcp_status_block(columns: int):
            if not isinstance(self.soul, ScreamSoul):
                return None
        ...
        with CustomPromptSession(
            ...
            model_name=model_display_name(
                ...
                if isinstance(self.soul, ScreamSoul) and self.soul.runtime.llm
                else None,
            ),
            ...
            editor_command_provider=lambda: (
                self.soul.runtime.config.default_editor if isinstance(self.soul, ScreamSoul) else ""
            ),
            ...
            if isinstance(self.soul, ScreamSoul):
                scream_soul = self.soul
                snapshot = scream_soul.status.mcp_status
                if snapshot and snapshot.loading:

                    async def _invalidate_after_mcp_loading() -> None:
                        try:
                            await scream_soul.wait_for_background_mcp_loading()
                        except Exception:
            ...
            try:
                while True:
                    ...
                    from scream.ui.shell.visualize import InputAction, classify_input
                    ...
                    if action.kind == InputAction.BTW and isinstance(self.soul, ScreamSoul):
        ...
        try:
            ...
            await run_soul(
                ...
                lambda wire: visualize(
                    ...
                    steer=self.soul.steer if isinstance(self.soul, ScreamSoul) else None,
    ...
    async def _run_btw_modal(
        ...
        """
        from scream.soul.btw import execute_side_question
        from scream.ui.shell.visualize import (
            _BtwModalDelegate,  # pyright: ignore[reportPrivateUsage]
        )

        assert isinstance(self.soul, ScreamSoul)
    ...
    def _set_active_view(self, view: Any) -> None:
        ...
        while self._pending_approval_requests:
            ...
            if not isinstance(self.soul, ScreamSoul) or self.soul.runtime.approval_runtime is None:
                break
            record = self.soul.runtime.approval_runtime.get_request(request.id)
            if record is None or record.status != "pending":
                continue
    ...
    def _forward_approval_to_sink(self, request: ApprovalRequest) -> None:
        ...
        async def _bridge() -> None:
            try:
                ...
                if (
                    isinstance(self.soul, ScreamSoul)
    ...
    async def _auto_update(self) -> None:
        ...
        if result == UpdateResult.UPDATE_AVAILABLE:
            toast("发现新版本，下次启动时提醒", topic="update", duration=5.0)
...
def _print_welcome_info(name: str, info_items: list[WelcomeInfoItem]) -> None:
    head = Text.from_markup("Welcome to Scream Code 你的中文Ai助手")
    help_text = Text.from_markup("[grey50]输入 /help 查看帮助信息。[/grey50]")
    ...
    if LATEST_VERSION_FILE.exists():
        ...
        from scream.ui.shell.update import SKIPPED_VERSION_FILE
        from scream.utils.envvar import get_env_bool

        if not get_env_bool("SCREAM_CLI_NO_AUTO_UPDATE"):
            try:
                ...
                if skipped != latest_version:
                    rows.append(
                        Text.from_markup(
                            ...
                            f"请运行 `{_update_mod.UPGRADE_COMMAND}` 进行升级。[/yellow]"
                        )
                    )
                    from scream.telemetry import track
