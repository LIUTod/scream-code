...
from scream.ui.shell.console import console, render_to_ansi
from scream.ui.shell.keyboard import KeyEvent
from scream.utils.rich.diff_render import (
    collect_diff_hunks,
    render_diff_panel,
    render_diff_preview,
...
from scream.utils.rich.syntax import ScreamSyntax
...
class ApprovalRequestPanel:
    ...
    def __init__(self, request: ApprovalRequest):
        ...
        self.options: list[tuple[str, ApprovalResponse.Kind]] = [
            ("仅批准一次", "approve"),
            ("本次会话始终批准", "approve_for_session"),
            ("拒绝", "reject"),
            ("拒绝，并告诉模型应该怎么做", "reject"),
        ]
            ...
            if isinstance(block, DiffDisplayBlock):
                ...
                if non_diff_budget > 0:
                    ...
                    if line_count > non_diff_budget:
                        truncated = "\n".join(text.split("\n")[:non_diff_budget])
                        self._non_diff_truncated = True
                    self._preview_renderables.append(ScreamSyntax(truncated, block.language))
                    non_diff_budget -= min(line_count, non_diff_budget)
                else:
                    self._non_diff_truncated = True
                idx += 1
    ...
    def render(
        ...
        *,
        ...
        content_lines: list[RenderableType] = [
            Text.from_markup(
                "[yellow]"
                f"{escape(self.request.sender)} 请求批准执行 "
        ...
        if self.has_expandable_content and self._non_diff_truncated:
            content_lines.append(Text("... (已截断，按 ctrl-e 展开)", style="dim italic"))
        ...
        if show_inline_feedback:
            hint = "  输入反馈内容，然后按 Enter 提交。"
        else:
            hint = "  \u25b2/\u25bc \u9009\u62e9  1/2/3/4 \u786e\u8ba4  \u21b5 \u63d0\u4ea4"
    ...
    def _render_block(
        ...
        if block.lexer:
            return ScreamSyntax(text, block.lexer)
    ...
    def _render_source_metadata_lines(self) -> list[RenderableType]:
        ...
        if self.request.subagent_type is not None or self.request.agent_id is not None:
            ...
            lines.append(Text(f"子代理: {subagent_text}", style="grey50"))
        if self.request.source_description:
            lines.append(Text(f"任务: {self.request.source_description}", style="grey50"))
        return lines
...
def show_approval_in_pager(panel: ApprovalRequestPanel) -> None:
    ...
    with console.screen(), console.pager(styles=True):
        console.print(
            Text.from_markup(
                "[yellow]⚠ "
                f"{escape(panel.request.sender)} 请求批准执行 "
        ...
        while idx < len(display):
            ...
            if isinstance(block, DiffDisplayBlock):
                ...
                console.print(ScreamSyntax(block.command.rstrip("\n"), block.language))
                rendered_any = True
                idx += 1
            elif isinstance(block, BriefDisplayBlock) and block.text:
