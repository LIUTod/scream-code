...
from scream.soul import StatusSnapshot, format_context_status
from scream.ui.shell import placeholders as prompt_placeholders
from scream.ui.shell.console import console
from scream.ui.shell.placeholders import (
    PromptPlaceholderManager,
    normalize_pasted_text,
    sanitize_surrogates,
)
from scream.ui.theme import get_prompt_style, get_toolbar_colors
from scream.utils.clipboard import (
...
from scream.wire.types import ContentPart
...
class LocalFileMentionCompleter(Completer):
    ...
    :mod:`scream.utils.file_filter` so that the web backend can reuse
    them.
    """
    ...
    def _get_top_level_paths(self) -> list[str]:
        from scream.utils.file_filter import is_ignored
    ...
    def _get_deep_paths(self) -> list[str]:
        from scream.utils.file_filter import (
            detect_git,
            git_index_mtime,
            list_files_git,
            list_files_walk,
...
class CustomPromptSession:
        ...
        def _(event: KeyPressEvent) -> None:
            ...
            if self._plan_mode_toggle_callback is not None:
                ...
                async def _toggle() -> None:
                    ...
                    if new_state:
                        toast("计划模式 开启", topic="plan_mode", duration=3.0, immediate=True)
                    else:
                        toast("计划模式 关闭", topic="plan_mode", duration=3.0, immediate=True)
                    event.app.invalidate()
    ...
    def _open_in_external_editor(self, event: KeyPressEvent) -> None:
        ...
        from scream.utils.editor import edit_text_in_editor, get_editor_command

        configured = self._editor_command_provider()

        if get_editor_command(configured) is None:
            toast("未找到编辑器。请设置 $VISUAL/$EDITOR 或运行 /editor。")
            return
