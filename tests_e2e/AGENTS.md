...
## Goals and Scope
- Test only `scream --wire` JSON-RPC + wire messages; no Shell UI/Print/ACP/Term/shortcuts.
- Do not test `--agent okabe`.
- Do not test: W-23, W-26, W-29, W-27 (env overrides).

## Execution Rules
- Tests run via `uv run scream` by default; set `SCREAM_E2E_WIRE_CMD` to override the base command
  (e.g. `../scream-agent-rs/target/debug/scream-agent` or `scream-agent` on PATH). `--wire` is appended if missing.
- Always isolate `HOME`, `USERPROFILE`, and `SCREAM_SHARE_DIR`, and use a temporary `--work-dir` to
  avoid touching real `~/.scream`.
