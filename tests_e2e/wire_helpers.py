...
TRACE_ENV = "SCREAM_TEST_TRACE"
WIRE_COMMAND_ENV = "SCREAM_E2E_WIRE_CMD"
DEFAULT_TIMEOUT = 5.0
...
def make_env(home_dir: Path) -> dict[str, str]:
    ...
    env["SCREAM_SHARE_DIR"] = str(share_dir(home_dir))
    return env


def share_dir(home_dir: Path) -> Path:
    return home_dir / ".scream"
    ...
    config_data: dict[str, Any] = {
        ...
        "providers": {
            provider_name: {
                ...
                "base_url": "",
                "api_key": "",
                "env": {"SCREAM_SCRIPTED_ECHO_SCRIPTS": str(scripts_path)},
            }
        },
    }
    if loop_control:
...
def _normalize_server_version(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        ...
        if value.get("name") == "Scream Code CLI" and "version" in value:
...
def base_command() -> list[str]:
    ...
    parts = shlex.split(override, posix=os.name != "nt") if override else ["uv", "run", "scream"]
