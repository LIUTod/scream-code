#!/usr/bin/env python3
"""Clean up .scream sessions whose workdir is under a temporary directory.

This script handles two cases:
  1. Entries in scream.json whose path is a tmp directory -> remove entry + session dir.
  2. Orphan session directories on disk that have no matching scream.json entry
...
SCREAM_DIR = Path.home() / ".scream"
METADATA_FILE = SCREAM_DIR / "scream.json"
SESSIONS_DIR = SCREAM_DIR / "sessions"
...
def main() -> None:
    ...
    # --- Phase 1: tmp entries in scream.json ---
    ...
    # --- Phase 2: orphan directories (on disk but not in scream.json) ---
