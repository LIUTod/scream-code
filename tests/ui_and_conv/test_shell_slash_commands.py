...
from scream.ui.shell.slash import (
    ShellSlashCmdFunc,
...
def isolated_share_dir(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    ...
    monkeypatch.setattr("scream.share.get_share_dir", _get_share_dir)
    monkeypatch.setattr("scream.metadata.get_share_dir", _get_share_dir)
    return share_dir
...
def mock_shell(work_dir: KaosPath) -> Mock:
    """Create a mock Shell whose soul passes the ScreamSoul isinstance check.
    ...
    mock_soul = Mock(spec=ScreamSoul)
...
class TestNewCommandBehavior:
    ...
    async def test_returns_early_without_scream_soul(self) -> None:
        """When soul is not a ScreamSoul, the command should silently return."""
        shell = Mock()
        shell.soul = Mock()  # plain Mock, not spec=ScreamSoul
