...
from scream.config import LLMProvider, OAuthRef
...
from scream.ui.shell import slash as shell_slash
from scream.ui.shell.slash import registry as shell_slash_registry
from scream.ui.shell.slash import shell_mode_registry
...
def _setup_feedback_provider(runtime: Runtime) -> None:
    """Add a managed:scream-code provider with OAuth to the runtime config."""
    runtime.config.providers["managed:scream-code"] = LLMProvider(
        type="scream",
        base_url="https://api.example.com/coding/v1",
        api_key=SecretStr("test-api-key"),
        oauth=OAuthRef(storage="file", key="oauth/scream-code"),
...
def _setup_submission_mocks(monkeypatch, *, feedback_text="Great tool!", session_factory=None):
    ...
    if session_factory is not None:
        monkeypatch.setattr("scream.utils.aiohttp.new_client_session", session_factory)
...
class TestFeedbackGuards:
    async def test_fallback_when_no_scream_soul(self, monkeypatch) -> None:
        """When soul is not ScreamSoul, should fallback to GitHub issues."""
        shell = Mock()
        shell.soul = Mock()  # not spec=ScreamSoul
    ...
    async def test_fallback_when_no_provider(
        ...
        """When managed:scream-code provider is missing, should fallback."""
...
class TestFeedbackSubmission:
    async def test_successful_submission(
        ...
        assert post_call.args[0] == "https://api.example.com/coding/v1/feedback"
