...
import pytest
from ltod.message import Message, ToolCall
from ltod.tooling import Tool, ToolError, ToolResult

from scream.soul.btw import (
    _build_btw_context,
    _DenyAllToolset,
    _tool_result_to_message,
    execute_side_question,
)
from scream.ui.shell.prompt import PromptMode, UserInput
...
# ---------------------------------------------------------------------------
# Helpers for mocking ltod.step
# ---------------------------------------------------------------------------


@dataclass
class _FakeStepResult:
    """Minimal stand-in for ltod.StepResult."""
...
class TestExecuteSideQuestion:
    ...
    def test_text_on_first_turn(self):
        ...
        with patch("scream.soul.btw.ltod.step", side_effect=fake_step):
            response, error = asyncio.run(execute_side_question(soul, "hi"))
...
class TestExecuteSideQuestionTelemetry:
    ...
    def test_tracks_success(self):
        ...
        with (
            patch("scream.soul.btw.ltod.step", side_effect=fake_step),
...
class TestSteerDedup:
    def test_local_steer_consumed_by_counter(self, monkeypatch):
        ...
        from scream.ui.shell.visualize import _LiveView
