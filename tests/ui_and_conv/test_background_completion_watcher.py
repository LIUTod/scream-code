...
from scream.ui.shell import Shell, _BackgroundCompletionWatcher, _PromptEvent
...
# -------------------------------------------------------------------
# Disabled watcher: non-ScreamSoul path
# -------------------------------------------------------------------


@pytest.mark.asyncio
async def test_disabled_watcher_just_awaits_idle():
    """When watcher is disabled (no ScreamSoul), it behaves as plain get()."""
    watcher = _BackgroundCompletionWatcher.__new__(_BackgroundCompletionWatcher)
    watcher._event = None
