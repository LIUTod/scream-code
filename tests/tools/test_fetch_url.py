...
from scream.tools.web.fetch import FetchURL, Params
...
async def test_fetch_url_404_url(fetch_url_tool: FetchURL) -> None:
    ...
    result = await fetch_url_tool(
        Params(url="https://github.com/LIUTod/non-existing-repo/issues/1")
    )
...
async def test_fetch_url_javascript_driven_site(fetch_url_tool: FetchURL) -> None:
    ...
    result = await fetch_url_tool(Params(url="https://www.example.com/"))
...
async def test_fetch_url_with_service(runtime) -> None:
    ...
    from scream.config import Config, FetchConfig, Services
