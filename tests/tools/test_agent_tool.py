...
import pytest
from ltod.chat_provider import APIConnectionError, APIStatusError, ChatProviderError
from ltod.message import Message
from ltod.tooling.empty import EmptyToolset

from scream.approval_runtime import get_current_approval_source_or_none
from scream.soul import MaxStepsReached, RunCancelled
from scream.soul.agent import Agent as SoulAgent
from scream.subagents import AgentLaunchSpec, AgentTypeDefinition, ToolPolicy
from scream.wire.types import ApprovalRequest, TextPart
...
async def test_agent_tool_creates_instance_and_returns_agent_id(agent_tool, runtime, monkeypatch):
    ...
    monkeypatch.setattr("scream.subagents.builder.load_agent", fake_load_agent)
    monkeypatch.setattr("scream.subagents.runner.run_soul", fake_run_soul)
...
async def test_foreground_agent_explicit_timeout_returns_tool_error(
    ...
    monkeypatch.setattr("scream.subagents.builder.load_agent", fake_load_agent)
    monkeypatch.setattr("scream.subagents.runner.run_soul", fake_run_soul_hang)
...
async def test_foreground_agent_internal_timeout_with_explicit_deadline(
    ...
    monkeypatch.setattr("scream.subagents.builder.load_agent", fake_load_agent)
    monkeypatch.setattr("scream.subagents.runner.run_soul", fake_run_soul_internal_timeout)
...
async def test_agent_tool_returns_error_when_final_response_is_none(
    ...
    monkeypatch.setattr(
        "scream.subagents.runner.run_with_summary_continuation",
...
async def test_background_agent_marks_failed_when_final_response_is_none(
    ...
    monkeypatch.setattr(
        "scream.background.agent_runner.run_with_summary_continuation",
        fake_run_with_summary,
    )
...
async def test_foreground_runner_hook_trigger_exception_marks_instance_failed(
    ...
    monkeypatch.setattr(
        "scream.hooks.events.subagent_start",
