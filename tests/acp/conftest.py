...
def _scream_bin() -> str:
    """Return the path to the scream entry-point script inside the venv."""
    return str(_repo_root() / ".venv" / "bin" / "scream")
...
def acp_share_dir(tmp_path: Path) -> Path:
    ...
    trace_env = os.getenv("SCREAM_SCRIPTED_ECHO_TRACE", "0")
    config_data = {
        ...
        "providers": {
            "scripted_provider": {
                ...
                "api_key": "",
                "env": {
                    "SCREAM_SCRIPTED_ECHO_SCRIPTS": str(scripts_path),
                    "SCREAM_SCRIPTED_ECHO_TRACE": trace_env,
                },
            }
        },
    }
    ...
    (credentials_dir / "scream-code.json").write_text(
        json.dumps(
            {
...
async def acp_client(
    ...
    """Spawn a scream ACP subprocess and return the SDK connection + test client."""
    test_client = ACPTestClient()
    env = {
        **os.environ,
        "SCREAM_SHARE_DIR": str(acp_share_dir),
    }
