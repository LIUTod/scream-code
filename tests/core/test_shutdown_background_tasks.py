"""Tests for ``ScreamCLI.shutdown_background_tasks``.
...
def _make_cli(
    *,
...
) -> tuple[ScreamCLI, MagicMock, dict]:
    """Build a ScreamCLI stub exposing only what shutdown_background_tasks needs.
    ...
    cli = ScreamCLI.__new__(ScreamCLI)
    cli._soul = MagicMock()
...
async def test_shutdown_prints_notice_and_kills_when_active(capsys) -> None:
    ...
    with patch("scream.app.asyncio.sleep", side_effect=fake_sleep):
...
async def test_shutdown_writes_to_original_stderr_when_redirected(monkeypatch) -> None:
    ...
    alone would silently send the notice into ``scream.log`` (where fd=2 now
    ...
    monkeypatch.setattr("scream.app.open_original_stderr", fake_open_original_stderr)
