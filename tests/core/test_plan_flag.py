"""Tests for --plan flag and default_plan_mode config in ScreamCLI.create()."""
...
import scream.app as app_module
from scream.app import ScreamCLI

# ---------------------------------------------------------------------------
# Helpers — lightweight fakes for ScreamCLI.create() dependencies
# ---------------------------------------------------------------------------


def _patch_create_deps(monkeypatch, *, session_plan_mode: bool = False):
    """Patch heavy dependencies so ScreamCLI.create() runs without I/O.

    Returns a FakeSoul class whose instances record plan-mode interactions.
    """

    class FakeSoul:
        """Tracks plan-mode calls made by ScreamCLI.create()."""
    ...
    monkeypatch.setattr(app_module, "ScreamSoul", FakeSoul)
...
class TestPlanFlagNewSession:
    ...
    async def test_plan_flag_activates_plan_mode(self, session, config, monkeypatch):
        ...
        await ScreamCLI.create(session, config=config, plan_mode=True, resumed=False)
    ...
    async def test_config_default_plan_mode_activates(self, session, config, monkeypatch):
        ...
        await ScreamCLI.create(session, config=config, plan_mode=False, resumed=False)
...
class TestPlanFlagResumedSession:
    ...
    async def test_config_not_applied_on_resumed_session(self, session, config, monkeypatch):
        ...
        await ScreamCLI.create(session, config=config, plan_mode=False, resumed=True)
    ...
    async def test_plan_flag_activates_on_resumed_session_without_plan(
        ...
        await ScreamCLI.create(session, config=config, plan_mode=True, resumed=True)
...
class TestPlanFlagPriority:
    ...
    async def test_default_is_false(self, session, config, monkeypatch):
        ...
        await ScreamCLI.create(session, config=config, resumed=False)
    ...
    async def test_plan_and_yolo_coexist(self, session, config, monkeypatch):
        ...
        await ScreamCLI.create(session, config=config, plan_mode=True, yolo=True, resumed=False)
    ...
    async def test_runtime_afk_passed_separately_from_afk(self, session, config, monkeypatch):
        ...
        await ScreamCLI.create(session, config=config, runtime_afk=True, resumed=False)
...
class TestSchedulePlanActivationReminder:
    """Unit tests for ScreamSoul.schedule_plan_activation_reminder()."""
    ...
    def test_schedules_when_in_plan_mode(self, runtime, tmp_path, monkeypatch):
        ...
        from scream.soul.agent import Agent
        ...
        soul = ScreamSoul(agent, context=Context(file_backend=tmp_path / "ctx.jsonl"))
