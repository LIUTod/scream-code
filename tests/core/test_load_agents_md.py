...
from scream.soul.agent import _AGENTS_MD_MAX_BYTES, load_agents_md
...
async def test_scream_dir_and_root_both_loaded(temp_work_dir: KaosPath):
    """.scream/AGENTS.md and AGENTS.md in the same dir are both loaded; .scream/ first."""
    scream_dir = temp_work_dir / ".scream"
    await scream_dir.mkdir()
    await (scream_dir / "AGENTS.md").write_text("scream agents")
    await (temp_work_dir / "AGENTS.md").write_text("root agents")

    content = await load_agents_md(temp_work_dir)

    assert content is not None
    assert content.index("scream agents") < content.index("root agents")
    assert content.count("<!-- From:") == 2


async def test_scream_dir_in_parent(temp_work_dir: KaosPath):
    """.scream/AGENTS.md in a parent directory is discovered via hierarchy."""
    await (temp_work_dir / ".git").mkdir()
    scream_dir = temp_work_dir / ".scream"
    await scream_dir.mkdir()
    await (scream_dir / "AGENTS.md").write_text("parent scream")
    ...
    assert content is not None
    assert "parent scream" in content
    assert "child root" in content
    assert content.index("parent scream") < content.index("child root")
