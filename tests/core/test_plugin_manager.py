...
from scream.plugin.manager import (
    collect_host_values,
    install_plugin,
    list_plugins,
    remove_plugin,
...
def test_install_plugin(tmp_path: Path):
    ...
    assert pj["runtime"]["host"] == "scream-code"
...
async def test_skill_discovery_includes_plugins_dir(tmp_path: Path, monkeypatch):
    ...
    from scream.skill import resolve_skills_roots
    ...
    # Point SCREAM_SHARE_DIR to tmp_path so get_plugins_dir() returns tmp_path/plugins
