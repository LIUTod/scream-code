...
import scream.ui.shell.startup as startup_module
...
@pytest.mark.asyncio
async def test_scream_create_reports_startup_phases(session, config, monkeypatch) -> None:
    phases: list[str] = []
    ...
    monkeypatch.setattr(app_module, "ScreamSoul", _FakeSoul)

    cli = await ScreamCLI.create(session, config=config, startup_progress=phases.append)

    assert isinstance(cli, ScreamCLI)
    assert phases == [
...
@pytest.mark.asyncio
async def test_scream_create_cleans_stale_running_foreground_subagents(
    ...
    await ScreamCLI.create(session, config=config)
