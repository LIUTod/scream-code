"""Tests for ScreamToolset hide/unhide and deduplication functionality."""
...
from ltod.tooling.error import ToolNotFoundError as LtodToolNotFoundError
from pydantic import BaseModel

from scream.soul.toolset import ScreamToolset
from scream.wire.types import ToolCall, ToolResult
...
def _make_toolset() -> ScreamToolset:
    ts = ScreamToolset()
    ts.add(DummyToolA())
    ts.add(DummyToolB())
    return ts
...
def _tool_names(ts: ScreamToolset) -> set[str]:
...
async def test_hidden_tool_still_handled():
    ...
    if isinstance(result, ToolResult):
        assert not isinstance(result.return_value, LtodToolNotFoundError)
    else:
...
async def test_nonexistent_tool_returns_not_found():
    ...
    assert isinstance(result.return_value, LtodToolNotFoundError)
...
def test_begin_end_step():
    ...
    # the caller (ScreamSoul) is expected to call begin_step again for the next step.
