...
from ltod.chat_provider.echo import EchoChatProvider
from ltod.chat_provider.scream import Scream
from ltod.contrib.chat_provider.openai_responses import OpenAIResponses
from pydantic import SecretStr

from scream.config import LLMModel, LLMProvider
from scream.llm import augment_provider_with_env_vars, create_llm


def test_augment_provider_with_env_vars_scream(monkeypatch):
    ...
    monkeypatch.setenv("SCREAM_BASE_URL", "https://env.test/v1")
    monkeypatch.setenv("SCREAM_API_KEY", "env-key")
    monkeypatch.setenv("SCREAM_MODEL_NAME", "scream-env-model")
    monkeypatch.setenv("SCREAM_MODEL_MAX_CONTEXT_SIZE", "8192")
    monkeypatch.setenv("SCREAM_MODEL_CAPABILITIES", "Image_In,THINKING,unknown")
...
def test_create_llm_scream_model_parameters(monkeypatch):
    provider = LLMProvider(
        type="scream",
    ...
    monkeypatch.setenv("SCREAM_MODEL_TEMPERATURE", "0.2")
    monkeypatch.setenv("SCREAM_MODEL_TOP_P", "0.8")
    monkeypatch.setenv("SCREAM_MODEL_MAX_TOKENS", "1234")

    llm = create_llm(provider, model)
    assert llm is not None
    assert isinstance(llm.chat_provider, Scream)
...
def test_create_llm_anthropic_with_session_id():
    from ltod.contrib.chat_provider.anthropic import Anthropic
...
def test_create_llm_requires_base_url_for_scream():
    provider = LLMProvider(type="scream", base_url="", api_key=SecretStr("test-key"))
    model = LLMModel(provider="scream", model="scream-base", max_context_size=4096)

    assert create_llm(provider, model) is None


def test_create_llm_openai_legacy_custom_headers():
    from ltod.contrib.chat_provider.openai_legacy import OpenAILegacy
...
def test_create_llm_google_genai_custom_headers():
    from ltod.contrib.chat_provider.google_genai import GoogleGenAI
...
def _make_scream_thinking_model() -> tuple[LLMProvider, LLMModel]:
    """Helper: build a scream provider + always-thinking model pair."""
    provider = LLMProvider(
        type="scream",
        base_url="https://api.test/v1",
        api_key=SecretStr("test-key"),
    ...
    model = LLMModel(
        provider="scream",
        model="scream-k2-thinking-turbo",
        max_context_size=4096,
        capabilities=None,
    )
...
def _make_scream_plain_model() -> tuple[LLMProvider, LLMModel]:
    """Helper: build a scream provider + non-thinking model pair."""
    provider = LLMProvider(
        type="scream",
        base_url="https://api.test/v1",
        api_key=SecretStr("test-key"),
...
def test_create_llm_scream_thinking_keep_not_set_omits_field(monkeypatch):
    """When SCREAM_MODEL_THINKING_KEEP is unset, extra_body.thinking must not
    contain a ``keep`` key, even for always-thinking models."""
    monkeypatch.delenv("SCREAM_MODEL_THINKING_KEEP", raising=False)
    provider, model = _make_scream_thinking_model()
...
def test_create_llm_scream_thinking_keep_empty_string_omits_field(monkeypatch):
    """An empty-string env value must be treated as unset (consistent with
    other SCREAM_MODEL_* envs that use walrus-truthy reads)."""
    monkeypatch.setenv("SCREAM_MODEL_THINKING_KEEP", "")
    provider, model = _make_scream_thinking_model()
...
def test_create_llm_scream_thinking_keep_all_injects_field(monkeypatch):
    """With a thinking-capable model and SCREAM_MODEL_THINKING_KEEP=all, the
    provider's extra_body.thinking must carry both ``type`` (set by
    with_thinking) and ``keep`` (set by the env)."""
    monkeypatch.setenv("SCREAM_MODEL_THINKING_KEEP", "all")
    provider, model = _make_scream_thinking_model()
...
def test_create_llm_scream_thinking_keep_arbitrary_value_passes_through(monkeypatch):
    """Non-'all' values must be forwarded unchanged — no casing normalization,
    no validation. The Scream API is the source of truth."""
    monkeypatch.setenv("SCREAM_MODEL_THINKING_KEEP", "xYz")
    provider, model = _make_scream_thinking_model()
...
def test_create_llm_scream_thinking_keep_skipped_when_thinking_off(monkeypatch):
    ...
    monkeypatch.setenv("SCREAM_MODEL_THINKING_KEEP", "all")
    provider, model = _make_scream_plain_model()
...
def test_create_llm_scream_thinking_keep_skipped_when_no_thinking_branch(monkeypatch):
...
def test_create_llm_scream_thinking_keep_injected_on_explicit_thinking_true(monkeypatch):
