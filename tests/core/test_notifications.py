...
import pytest
from ltod.chat_provider import StreamedMessagePart, ThinkingEffort, TokenUsage
from ltod.message import Message, TextPart
from ltod.tooling.empty import EmptyToolset

from scream.app import ScreamCLI
from scream.approval_runtime import ApprovalSource, get_current_approval_source_or_none
from scream.background import TaskRuntime, TaskSpec
from scream.llm import LLM
from scream.notifications import NotificationEvent
from scream.soul import RunCancelled, StatusSnapshot, _current_wire, run_soul
...
from scream.wire.types import ApprovalRequest, ApprovalResponse, Notification
...
def _make_soul(runtime: Runtime, tmp_path: Path) -> tuple[ScreamSoul, Context]:
    llm = LLM(
    ...
    return ScreamSoul(agent, context=context), context
...
@pytest.mark.asyncio
async def test_screamsoul_appends_notification_message(runtime: Runtime, tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_scream_run_yields_root_hub_approvals(runtime: Runtime) -> None:
    ...
    cli = ScreamCLI(_ApprovalOnlySoul(runtime), runtime, {})  # type: ignore[arg-type]
...
@pytest.mark.asyncio
async def test_scream_run_bridges_approval_resolution_back_to_runtime(runtime: Runtime) -> None:
    ...
    cli = ScreamCLI(soul, runtime, {})  # type: ignore[arg-type]
...
@pytest.mark.asyncio
async def test_scream_run_cancels_abandoned_approval_stream(
    runtime: Runtime,
    tmp_path: Path,
    ...
    monkeypatch.setattr(ScreamSoul, "_turn", fake_turn)
...
@pytest.mark.asyncio
async def test_scream_run_propagates_external_cancel_event(
    runtime: Runtime,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_scream_run_replays_pending_approvals_from_previous_turn(runtime: Runtime) -> None:
    assert runtime.approval_runtime is not None
