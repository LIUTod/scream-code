...
import pytest
from ltod.tooling import ToolError, ToolReturnValue
...
from scream.tools.plan import ExitPlanMode, Params, PlanOption
from scream.tools.plan.enter import _DESCRIPTION, EnterPlanMode
from scream.tools.plan.heroes import (
    _slug_cache,
    get_or_create_slug,
    get_plan_file_path,
    read_plan_file,
...
from scream.wire.types import QuestionNotSupported, QuestionRequest, ToolCall
...
class TestGetOrCreateSlug:
    def test_returns_hero_name(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("scream.tools.plan.heroes.PLANS_DIR", tmp_path)
    ...
    def test_collision_fallback(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("scream.tools.plan.heroes.HERO_NAMES", ["a", "b"])
...
def _mock_wire_and_tool_call(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("scream.tools.plan.get_wire_or_none", lambda: wire_mock)
    monkeypatch.setattr("scream.tools.plan.wire_send", lambda msg: None)
    monkeypatch.setattr("scream.tools.plan.enter.get_wire_or_none", lambda: wire_mock)
    monkeypatch.setattr("scream.tools.plan.enter.wire_send", lambda msg: None)
    tc = ToolCall(id="test-tc", function=ToolCall.FunctionBody(name="ExitPlanMode", arguments=None))
    monkeypatch.setattr("scream.tools.plan.get_current_tool_call_or_none", lambda: tc)
    monkeypatch.setattr("scream.tools.plan.enter.get_current_tool_call_or_none", lambda: tc)
    return wire_mock
...
class TestExitPlanModeHappyPaths:
    ...
    async def test_wire_unavailable(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        tool, _, _ = _setup_exit_tool(tmp_path)
        monkeypatch.setattr("scream.tools.plan.get_wire_or_none", lambda: None)
...
class TestEnterPlanModeHappyPaths:
    ...
    async def test_wire_unavailable(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        tool, _, _ = _setup_enter_tool(tmp_path)
        monkeypatch.setattr("scream.tools.plan.enter.get_wire_or_none", lambda: None)
...
# ---------------------------------------------------------------------------
# ScreamSoul — plan mode state management
# ---------------------------------------------------------------------------


class TestScreamSoulPlanState:
...
# ---------------------------------------------------------------------------
# ScreamSoul — plan_session_id cross-process persistence
# ---------------------------------------------------------------------------


class TestScreamSoulPlanSessionPersistence:
    """Tests that plan_session_id survives simulated process restarts."""

    async def test_plan_session_id_survives_restart(
        self, runtime: Runtime, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Second ScreamSoul created from same session state gets same plan file path."""
        monkeypatch.setattr("scream.tools.plan.heroes.PLANS_DIR", tmp_path)
        ...
        # New ScreamSoul reads from same (already-saved) session state and re-seeds cache
    ...
    async def test_plan_session_id_cleared_after_deactivation(
        ...
        # Restart + re-activate
        from scream.tools.plan.heroes import _slug_cache
