...
from scream.wire.file import WireMessageRecord
from scream.wire.serde import deserialize_wire_message, serialize_wire_message
...
async def test_wire_message_serde():
    ...
    msg = PlanDisplay(
        content="## Plan\n\n1. Step one\n2. Step two",
        file_path="/Users/test/.scream/plans/iron-man-spider-man.md",
    )
    assert serialize_wire_message(msg) == snapshot(
        {
            "type": "PlanDisplay",
...
def test_wire_message_type_alias():
    import scream.wire.types

    module = scream.wire.types
    # Helper types that are BaseModel subclasses but not WireMessage types
    from scream.wire.types import HookResponse
