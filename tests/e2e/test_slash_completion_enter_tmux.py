...
def _start_tmux_shell(
    *,
    ...
    env = {
        ...
        "SCREAM_SHARE_DIR": str(home_dir / ".scream"),
        "SCREAM_CLI_NO_AUTO_UPDATE": "1",
        "TERM": "xterm-256color",
        "COLUMNS": str(columns),
        "LINES": str(lines),
        "PYTHONUTF8": "1",
...
def test_slash_completion_single_enter_executes(tmp_path: Path) -> None:
    ...
    session_name = f"scream-tmux-slash-{uuid.uuid4().hex[:8]}"
    ...
    try:
        ...
        _wait_for_pane_text(session_name, "Welcome to Scream Code CLI!")
