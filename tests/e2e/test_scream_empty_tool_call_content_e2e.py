"""E2E regression test for Scream-compatible endpoints that reject empty assistant text.
...
3. Some Scream-compatible gateways reject the second request if the prior assistant
...
class MockScreamCompatServer(Protocol):
    base_url: str
    requests: list[dict[str, Any]]


class _MockScreamCompatServer:
...
async def mock_scream_compat_server() -> AsyncIterator[MockScreamCompatServer]:
    server_holder: _MockScreamCompatServer | None = None
    ...
    server_holder = _MockScreamCompatServer(f"http://127.0.0.1:{port}")
...
def _write_scream_config(config_path: Path, *, base_url: str) -> None:
    config_path.write_text(
        json.dumps(
            {
                "default_model": "mock-scream",
                "models": {
                    "mock-scream": {
                        "provider": "mock-scream-provider",
                        "model": "scream-for-coding",
                        "max_context_size": 100000,
                    }
                },
                "providers": {
                    "mock-scream-provider": {
                        "type": "scream",
                        "base_url": base_url,
...
async def _run_scream_print_json(
    *,
    config_path: Path,
    share_dir: Path,
    work_dir: Path,
    ...
    env = os.environ.copy()
    env["SCREAM_SHARE_DIR"] = str(share_dir)
    env["SCREAM_DISABLE_TELEMETRY"] = "1"
    env["COLUMNS"] = "120"
    env["LINES"] = "40"
...
async def test_scream_compat_endpoint_accepts_tool_call_history_without_empty_content(
    tmp_path: Path, mock_scream_compat_server: MockScreamCompatServer
) -> None:
    ...
    _write_scream_config(config_path, base_url=f"{mock_scream_compat_server.base_url}/v1")

    return_code, stdout, stderr = await _run_scream_print_json(
    ...
    assert len(mock_scream_compat_server.requests) == 2
    assistant_message = _find_assistant_tool_call_message(mock_scream_compat_server.requests[1])
